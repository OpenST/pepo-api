...
class ReValidateAllReceipts extends CronBase {
    ...
    while (oThis.areRowsRemainingToProcess) {
      ...
      if (dbRows.length === 0) {
        ...
        for (let i = 0; i < dbRows.length; i++) {
          logger.log('Operation initiated for payment id: ', dbRows[i].id);
  ...
  async _operateOnFetchedRow(dataRow) {
    ...
    let getPaymentValidatorObj = paymentValidationFactory.getInstance(os, params),
