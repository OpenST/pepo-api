...
class CommonValidator {
  ...
  static validateNonBlankStringArray(array) {
    if (Array.isArray(array)) {
      for (let i = 0; i < array.length; i++) {
        ...
        if (CommonValidator.isVarNull(variable) || !CommonValidator.validateString(variable) || variable == '') {
          return false;
        }
      }
      return true;
  ...
  static validateOstTransactionMetaText(variable) {
    ...
    return (
      ...
      CommonValidator.validateMaxLengthMediumString(variable) &&
      /^[a-z0-9\_\.\s]+$/i.test(variable)
    );
  }
  ...
  static validateOstTransactionMetaGiphy(variable) {
    ...
    if (
      CommonValidator.isVarNull(id) ||
      CommonValidator.isVarNull(kind) ||
      CommonValidator.isVarNull(downsized) ||
      CommonValidator.isVarNull(fixedWidthDownsampled)
    ) {
      return false;
    }
  ...
  static validateOstTransactionMetaGiphySizeData(variable) {
    ...
    if (
      ...
      (!CommonValidator.isVarNull(frames) && !CommonValidator.validateNonNegativeInteger(frames)) ||
      !CommonValidator.validateGiphyUrl(mp4) ||
      (!CommonValidator.isVarNull(mp4_size) && !CommonValidator.validateNonNegativeInteger(mp4_size)) ||
      !CommonValidator.validateGiphyUrl(webp) ||
      (!CommonValidator.isVarNull(webp_size) && !CommonValidator.validateNonNegativeInteger(webp_size))
    ) {
      return false;
    }
  ...
  static validateFeedPrivacyType(variable) {
    return feedConstants.invertedPrivacyTypes[variable.toUpperCase()] !== undefined;
  }
  ...
  static validateOstTransactionObject(variable) {
    ...
    if (!CommonValidator.isVarNull(variable.transaction_hash)) {
      if (!CommonValidator.validateTransactionHash(variable.transaction_hash)) {
        return false;
      }
    }
    if (!CommonValidator.isVarNull(variable.from)) {
    ...
    if (!CommonValidator.isVarNull(variable.nonce)) {
      if (!CommonValidator.validateInteger(variable.nonce)) {
        return false;
      }
    }
    if (!CommonValidator.isVarNull(variable.value)) {
    ...
    if (!CommonValidator.isVarNull(variable.gas_used)) {
      if (!CommonValidator.validateInteger(variable.gas_used)) {
        return false;
      }
    }
    if (!CommonValidator.isVarNull(variable.transaction_fee)) {
      if (!CommonValidator.validateNonZeroWeiValue(variable.transaction_fee)) {
        return false;
      }
    }
    if (!CommonValidator.isVarNull(variable.block_confirmation)) {
    ...
    if (!CommonValidator.isVarNull(variable.block_timestamp)) {
      if (!CommonValidator.validateNonZeroInteger(variable.block_timestamp)) {
        return false;
      }
    }
    if (!CommonValidator.isVarNull(variable.block_number)) {
      if (!CommonValidator.validateNonZeroInteger(variable.block_number)) {
        return false;
      }
    }
    if (!CommonValidator.isVarNull(variable.rule_name)) {
