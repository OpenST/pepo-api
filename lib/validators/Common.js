const urlParser = require('url'),
  ...
  HtmlEntitiesClass = require('html-entities').AllHtmlEntities,
  htmlEntities = new HtmlEntitiesClass();
...
class CommonValidator {
  ...
  /**
   * Check if variable is object and non-empty. returns true if object is not empty else returns false.
   *
   ...
   * @returns {boolean}
   */
  static validateChannelName(string) {
    const trimmedString = string.trim();

    if (CommonValidator.validateNonEmptyString(trimmedString)) {
   ...
   * @returns {boolean}
   */
  static validateChannelNameLength(string) {
    const trimmedString = string.trim();
    const decodedString = htmlEntities.decode(trimmedString);

    return decodedString.length < 25;
  }
   ...
   * @returns {boolean}
   */
  static validateChannelNameAllowedCharacters(string) {
   ...
   * @returns {boolean}
   */
  static validateChannelTagline(string) {
    return CommonValidator.validateNonEmptyString(string) && string.length < 45;
  }

  /**
   * Validate channel tagline length.
   *
   * @param {string} string
   *
   * @returns {boolean}
   */
  static validateChannelTaglineLength(string) {
    const trimmedString = string.trim();

    return trimmedString.length < 45;
  }
   ...
   * @returns {boolean}
   */
  static validateChannelDescription(string) {
    return CommonValidator.validateNonEmptyString(string) && string.length < 400;
  }
   ...
   * @returns {boolean}
   */
  static validateChannelDescriptionLength(string) {
    const trimmedString = string.trim();

    return trimmedString.length < 400;
  }
   ...
   * @returns {boolean}
   */
  static validateNonEmptyChannelTagsArray(array) {
    if (Array.isArray(array)) {
  ...
  /**
   * Validate length of channel tags array.
   *
   * @param {array} array
   *
   * @returns {boolean}
   */
  static validateChannelTagsArrayLength(array) {
    if (Array.isArray(array)) {
  ...
  /**
   * Validate individual channel tag length.
   *
   * @param {string} string
   *
   * @returns {boolean}
   */
  static validateChannelTagLength(string) {
    if (CommonValidator.validateNonEmptyString(string)) {
      const trimmedString = string.trim();

      return trimmedString.length <= 60;
    }
  ...
  /**
   * Validate stringified non empty array.
   *
   * @param {string} string
   *
   * @returns {boolean}
   */
  static validateStringifiedNonEmptyArray(string) {
    if (CommonValidator.validateCommaDelimitedStrings(string)) {
      const channelTagsArray = string.split(',');

      return CommonValidator.validateNonEmptyArray(channelTagsArray);
    }
  ...
  /**
   * Validate channel cover image file size. It should not be greater than 3MB.
   *
   * @param {number} fileSizeInBytes
   *
   * @returns {boolean}
   */
  static validateChannelCoverImageFileSize(fileSizeInBytes) {
    return CommonValidator.validateNonZeroInteger(fileSizeInBytes) && Number(fileSizeInBytes) <= 3145728; // 3MB.
  }

  /**
   * Validate channel cover image height. It should be at least 642 pixels.
   *
   * @param {number} heightInPixels
   *
   * @returns {boolean}
   */
  static validateChannelCoverImageHeight(heightInPixels) {
    return CommonValidator.validateNonZeroInteger(heightInPixels) && Number(heightInPixels) >= 642; // 642 pixels is the minimum height.
  }

  /**
   * Validate channel cover image width. It should be at least 1500 pixels.
   *
   * @param {number} widthInPixels
   *
   * @returns {boolean}
   */
  static validateChannelCoverImageWidth(widthInPixels) {
    return CommonValidator.validateNonZeroInteger(widthInPixels) && Number(widthInPixels) >= 1500; // 1500 pixels is the minimum width.
  }
  ...
  static isExternalLinkSafe(url) {
    let regexpHref = new RegExp(/ href=/gi),
      regexpRel = new RegExp(/ rel=/gi),
      regexpTarget = new RegExp(/ target=/gi);

    let totalHref = 0,
      totalRel = 0,
      totalTarget = 0;

    if (url.match(regexpHref)) {
      totalHref = url.match(regexpHref).length;
    }

    if (url.match(regexpRel)) {
      totalRel = url.match(regexpRel).length;
    }

    if (url.match(regexpTarget)) {
      totalTarget = url.match(regexpTarget).length;
    }

    if (totalHref == totalRel && totalHref == totalTarget) {
      return true;
    }
  ...
  /**
   * Checks for comma delimited strings.
   *
   * @param {string} variable
   *
   * @returns {boolean}
   */
  static validateCommaDelimitedStrings(variable) {
    return /[0-9a-zA-Z]+(,[0-9a-zA-Z]+)*/i.test(variable);
  }
}
