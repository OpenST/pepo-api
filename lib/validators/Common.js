...
class CommonValidator {
   ...
   * @returns {boolean}
   */
  static validateWeiValue(variable) {
    return CommonValidator.validateZeroWeiValue(variable) || CommonValidator.validateNonZeroWeiValue(variable);
  }
  ...
  static validateGenericUrl(url) {
    ...
    return /^(http(s)?:\/\/)?([a-zA-Z0-9-_@:%+~#=]{1,256}\.)+[a-z]{2,7}\b([a-zA-Z0-9@:%_+.\[\]\-{}!'",~#?&;/=*]*)$/i.test(
      url
    );
  }
