...
class CommonValidator {
  ...
  static validateInviteCode(variable) {
    ...
    return /^[a-z0-9]{6}$/i.test(variable);
  }
  ...
  static validateGenericUrl(url) {
    ...
    return /^(http(s)?:\/\/)?([a-zA-Z0-9-_@:%+~#=]{1,256}\.)+[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&;/=*]*)$/i.test(url);
  }
  ...
  static validateHttpBasedUrl(details) {
    ...
    return /^http(s)?:\/\/([a-zA-Z0-9-_@:%+~#=]{1,256}\.)+[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&;/=*]*)$/i.test(details);
  }
