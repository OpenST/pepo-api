...
class CommonValidator {
  ...
  static validateNextPagePayload(variable) {
    ...
    if (variable.limit) {
      notValid = notValid || !CommonValidator.validateNonZeroInteger(variable.limit);
    }
    if (variable.page) {
  ...
  static validateOstTransactionMetaGiphy(variable) {
    ...
    if (
      ...
      CommonValidator.isVarNull(original)
    ) {
      return false;
    }
  ...
  static validateOstTransactionMetaGiphySizeData(variable) {
    ...
    if (
      !CommonValidator.validateNonBlankStringArray([media_id, rendition_type, url, mp4, webp]) ||
      !CommonValidator.validateGiphyUrl(url) ||
      !CommonValidator.validateNonZeroInteger(width) ||
      !CommonValidator.validateNonZeroInteger(height) ||
      !CommonValidator.validateNonZeroInteger(size) ||
      (!CommonValidator.isVarNull(frames) && !CommonValidator.validateNonNegativeInteger(frames)) ||
      !CommonValidator.validateGiphyUrl(mp4) ||
      !CommonValidator.validateNonZeroInteger(mp4_size) ||
      !CommonValidator.validateGiphyUrl(webp) ||
      !CommonValidator.validateNonZeroInteger(webp_size)
    ) {
      return false;
    }
  ...
  static validateGiphyUrl(variable) {
    if (CommonValidator.isVarNull(variable) || !CommonValidator.validateString(variable)) {
      return false;
    }
    ...
    try {
      ...
      return /^https:\/\/media[a-z0-9]*\.giphy\.com$/i.test(obj.host);
    } catch (err) {
      return false;
    }
  }
