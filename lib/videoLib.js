...
class VideoLib {
  ...
  async validateAndSave(params) {
    ...
    if (params.videoKind === videoConstants.postVideoKind) {
      ...
      let replyInsertResp = await new ReplyDetailModel().insertVideo({
        userId: params.userId,
    ...
    // TODO - replies - there should be an else which throws error.
    ...
    return responseHelper.successWithData({ video: videoObject, videoId: videoId, replyDetailId: replyDetailId });
  }
