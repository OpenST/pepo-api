...
class VideoLib {
  ...
  async validateAndSave(params) {
    ...
    const videoInsertParams = {
      ...
      extraData: oThis._videoExtraData(params)
    };
  ...
  /**
   * Extra Data For Video
   *
   * @param params
   * @returns {string}
   * @private
   */
  _videoExtraData(params) {
    let extraData = { dp: Number(params.durationPref) || 30 };
    if (Number(params.duration)) {
      extraData.d = Number(params.duration);
    }
