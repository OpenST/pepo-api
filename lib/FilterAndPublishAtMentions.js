...
class FilterAndPublishAtMentions {
  ...
  async perform() {
    ...
    console.log('oThis.mentionedUserIds----------', oThis.mentionedUserIds);
    console.log('oThis.userIdsToRemoved----------', oThis.userIdsToRemoved);

    await oThis._insertInTextIncludes();
    await oThis.deleteOldUserIdsFromCassandra();
  ...
  async _filterAtMentions() {
    ...
    console.log('userNamesToUserIdMap-----', userNamesToUserIdMap);
   ...
   * @private
   */
  async deleteOldUserIdsFromCassandra() {
    const oThis = this;
