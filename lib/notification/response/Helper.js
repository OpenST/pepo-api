...
class NotificationHelper {
  ...
  static getGotoForNotification(params) {
    ...
    let gotoParams = {};
    for (let gotoParam in gotoConfig['params']) {
      let dataKeys = Object.values(gotoConfig['params']);
      gotoParams[gotoParam] = NotificationHelper.getKeyDataFromNotification(userNotification, dataKeys);
    }

    console.log('gotoParams-----', gotoConfig.kind, gotoParams);

    let finalGoto = gotoFactory.gotoFor(gotoConfig.kind, gotoParams);
  ...
  static getHeadingForNotification(params) {
    ...
    for (let index = 0; index < headingVars.length; index++) {
      ...
      let replaceText = resp.data.replaceText;
