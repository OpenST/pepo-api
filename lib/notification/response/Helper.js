...
class NotificationResponseGet {
  ...
  static getPayloadDataForNotification(params) {
    const userNotification = params.userNotification,
      kind = userNotification.kind,
      payload = {},
      payloadConfig = NotificationResponseGet.payloadNotificationConfigForKind(kind);
  ...
  static imageNotificationConfigForKind(kind) {
    const config = NotificationResponseGet.notificationCentreConfigForKind(kind);

    const imageConfig = config.image;

    if (!imageConfig) {
      throw new Error(`l_n_h_g_incfk_1: Invalid kind: ${kind}`);
    }
  ...
  static payloadNotificationConfigForKind(kind) {
    ...
    if (!payloadConfig) {
      throw new Error(`l_n_h_g_pncfk_1: Invalid kind-${kind}`);
    }
  ...
  static getSupportingEntitiesConfigForKind(kind) {
    ...
    if (!supportingEntitiesConfig) {
      throw new Error(`l_n_h_g_gsecfk_1: Invalid kind-${kind}`);
    }
  ...
  static headingNotificationConfigForKind(kind, headingVersion) {
    ...
    if (!headingConfig) {
      throw new Error(`l_n_h_g_hncfk_1: Invalid kind-${kind}, headingVersion:${headingVersion}`);
    }
  ...
  static gotoNotificationConfigForKind(kind) {
    ...
    if (!gotoConfig) {
      throw new Error(`l_n_h_g_gncfk_1: Invalid kind-${kind}`);
    }
   ...
   * @returns {*}
   */
  static notificationCentreConfigForKind(kind) {
    const config = notificationResponseConfig[kind];

    if (!config || !config.notificationCentre) {
      throw new Error(`l_n_h_g_ncfk_1: Invalid kind-${kind}`);
    }

    return config.notificationCentre;
  }
}
