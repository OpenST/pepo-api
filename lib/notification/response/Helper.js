const rootPrefix = '../../..',
  ...
  pushNotificationConfig = require(rootPrefix + '/lib/notification/config/pushNotification'),
...
class NotificationHelper {
  ...
  static getGotoForNotification(params) {
    ...
    for (let internalKey in gotoConfig['v']) {
      ...
      finalGoto['v'][internalKey] = NotificationHelper.getKeyDataFromNotification(userNotification, dataKeys);
    }
  ...
  static getHeadingForPushNotification(params) {
    const userNotification = params.userNotification,
      supportingEntities = params.supportingEntities,
      headingVersion = userNotification.headingVersion, // todo - modify
      notificationType = params.notificationType,
      kind = userNotification.kind;
    ...
    if (!basicHelper.isEmptyObject(replaceText)) {
      formattedText = title.replace(new RegExp(regex.join('|'), 'g'), function(word) {
        return replaceText[word];
      });
    }
    ...
    logger.log('GetHeadingForPushNotification for  kind:', kind, 'is :', finalResp);
  ...
  static headingNotificationConfigForKind(kind, headingVersion, notificationType) {
    let headingConfig = {},
      config = NotificationHelper.getConfigForKindAndNotificationType(kind, notificationType);
