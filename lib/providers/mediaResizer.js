...
class MediaResizer {
  ...
  resizeImage(requestData) {
    ...
    return oThis._sendPost('resize-image', requestData);
  }
  ...
  compressVideo(requestData) {
    ...
    return oThis._sendPost('compress-video', requestData);
  }
  ...
  async _sendPost(path, requestData) {
    ...
    logger.log('Calling Pepo resizer API POST: ' + completeUrl + '\n\n' + JSON.stringify(requestData));

    const response = await new HttpLibrary({ resource: completeUrl }).post(requestData);
    ...
    if (!responseData.success) {
      return responseHelper.error({
        internal_error_identifier: 'l_p_mr_1',
