const rootPrefix = '../..',
  ...
  PushNotificationsList = require(rootPrefix + '/app/services/user/notification/PushList'),
...
class PushNotificationProcessor {
  ...
  async _formatPayload() {
    ...
    let pushNotificationsListRsp = await new PushNotificationsList({
      notificationHookPayloads: [oThis.rawPayload],
      kind: oThis.eventType
    }).perform();

    oThis.messageParams = pushNotificationsListRsp.data.formattedNotifications[0];
