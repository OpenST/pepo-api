...
class PushNotificationProcessor {
        ...
        let deviceToken = oThis.userDeviceDetails[userDeviceId].deviceToken,
          ...
          message = Object.assign(oThis.messageParams, {
            ...
            apns: {
              payload: {
                aps: {
                  badge: oThis.userIdToBadgeCountMap[userId]
                }
              }
            }
          });
  ...
  async _formatPayload() {
    ...
    let pushNotificationRsp = await new PushNotificationMsgFormatter({
      notificationHookPayload: oThis.rawPayload,
      kind: oThis.eventType
    }).perform();

    if (pushNotificationRsp.isFailure()) {
      return Promise.reject(
        responseHelper.error({
    ...
    oThis.messageParams = pushNotificationRsp.data.formattedPayload;
    oThis.userIdToBadgeCountMap = pushNotificationRsp.data.userIdToBadgeCountMap;

    console.log('oThis.messageParam----', oThis.messageParams);
  }
}
