...
class FirebaseAdminSDK {
  ...
  async _initializeFireBaseObj() {
    ...
    fireBaseAdminSDK.initializeApp({
      credential: fireBaseAdminSDK.credential.cert(config),
      databaseURL: 'https://myapp2-3e902.firebaseio.com' // move to some other location.
    });

    oThis.fireBaseAdminSdkObj = fireBaseAdminSDK.initializeApp(firebaseConfig);
  }
  ...
  async sendNotification(registrationToken, message) {
    ...
    let sendNotificationRsp = oThis.fireBaseAdminSdkObj
      .messaging()
      .send(params)
      .catch(function(err) {
  ...
  async sendAllNotification(registrationToken, messages) {
    ...
    let sendAllNotificationRsp = oThis.fireBaseAdminSdkObj
      .messaging()
      .sendAll(params)
      .catch(function(err) {
        return oThis._errorResponseHandler(err);
  ...
  async sendMulticastNotification(registrationTokens, message) {
    ...
    let sendMulticastNotificationRsp = oThis.fireBaseAdminSdkObj
      .messaging()
