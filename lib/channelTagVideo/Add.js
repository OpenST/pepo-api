...
class AddChannelTagVideo {
  ...
  async perform() {
    ...
    await oThis._createChannelIdsAndVideoIdsArray();
   ...
   * @private
   */
  async _createChannelIdsAndVideoIdsArray() {
    const oThis = this;
    ...
    const channelCacheResponse = await new ChannelByIdsCache({ ids: allChannelIds }).fetch();
    if (channelCacheResponse.isFailure()) {
      return Promise.reject(channelCacheResponse);
    }
    oThis.channelMap = channelCacheResponse.data;

    for (const channelId in oThis.channelIdToVideoTagsMap) {
      if (oThis.channelMap[channelId].status !== channelConstants.activeStatus) {
        continue;
      }
      ...
      for (const videoId in videoIdToTagIdsArray) {
        ...
        for (let index = 0; index < channelVideoTags.length; index++) {
          const tagId = channelVideoTags[index],
            ...
            createdAt = oThis.videoIdTagIdToVideoDetailsMap[videoTagKey].createdAt;
  ...
  async _insertInChannelVideo() {
    ...
    for (let index = 0; index < oThis.insertChannelIds.length; index++) {
      ...
      for (let ind = 0; ind < videoIds.length; ind++) {
        ...
        const createdAt = oThis.videoIdTagIdToVideoDetailsMap[videoTagKey].createdAt;
