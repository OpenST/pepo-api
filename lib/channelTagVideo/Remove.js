...
/*
NOTE - We can query channel_videos for active channelIds and videoIds combinations. This would help us prevent
...
3. If entry found in the previous query matches the activeStatus records from the first query, ignore them.
   If not found, mark those channelId and videoId entries as inactive in channel_videos table.
4. Update channel_stats table according to the results from query 3.
 */

/**
 * After a channel is deleted:
 * - New channel_videos and new channel_tag_videos will keep getting removed.
 * - This makes sure if channel is activated again, channel_videos and channel_tag_videos will not have
 *   old/stale videos
 * - channel_stats will also be updated as per above logic.
 */
...
class RemoveChannelTagVideo {
  ...
  _createPrerequisiteData() {
    ...
    for (const channelId in oThis.channelIdToVideoTagsMap) {
      ...
      oThis.channelIdToTagIdsMap[channelId] = basicHelper.uniquate(oThis.channelIdToTagIdsMap[channelId]);
    }
  ...
  _getChannelVideosToMarkInactive() {
    ...
    oThis.channelIdsToBeAffected = basicHelper.uniquate(oThis.channelIdsToBeAffected);
  }
