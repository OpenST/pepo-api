...
class RemoveChannelTagVideo {
  ...
  async perform() {
    ...
    const promisesArray = [oThis._fetchActiveChannelVideos(), oThis._deleteFromChannelTagVideos()]; // Step 1, Step 2.
    await Promise.all(promisesArray);

    await oThis._fetchChannelTagVideos(); // Step 3.

    oThis._getChannelVideosToMarkInactive(); // Intermediate step.

    await oThis._markChannelVideosInactive(); // Step 4.

    await oThis._updateChannelStats(); // Step 5.
  }

  /**
   * Create channelIds and videoIds array.
   *
   * @sets oThis.channelIds, oThis.channelIdToTagIdsMap, oThis.channelIdToTagIdsMap
   ...
   *
   * @sets oThis.activeChannelVideos
   *
