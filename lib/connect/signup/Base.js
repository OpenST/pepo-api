...
class SignupBase {
  ...
  async perform() {
    ...
    const promiseArray2 = [
      oThis._socialConnectTableSpecificFunction(),
   ...
   * @private
   */
  async _socialConnectTableSpecificFunction() {
    throw new Error('Sub-class to implement.');
  }
  ...
  async _serviceResponse() {
    ...
    const socialAccountInfo = oThis._getFormattedSocialAccountData();
    ...
    return responseHelper.successWithData({
      ...
      socialAccountInfo: socialAccountInfo, // Changed key, it was twitterUserExtended before.
  ...
  /**
   * Get formatted social account data.
   *
   * @private
   */
  _getFormattedSocialAccountData() {
    throw new Error('Sub-class to implement.');
  }
}
