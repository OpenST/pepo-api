...
class SignupByApple extends SignupBase {
   ...
   * @param {string} params.appleUserEntity: User Entity Of apple
  ...
  constructor(params) {
    ...
    oThis.appleUserObj = params.appleUserObj || null;
  }

  /**
   * Get profile image url. Apple does not have profile image.
   *
   * @returns {null|*|string}
   * @private
   */
  ...
  _getSocialUserName() {
    ...
    return oThis.appleUserEntity.socialUserName;
  }
  ...
  async _createUpdateAppleUser() {
    ...
    // If Apple user obj is already present, means user has tried to do signup from web
    // In that case just update user_id in apple user obj.
    if (oThis.appleUserObj && !oThis.appleUserObj.userId) {
  ...
  async _createAppleUserExtended() {
    ...
    let encryptedAccessToken = localCipher.encrypt(oThis.decryptedEncryptionSalt, oThis.appleOAuthDetails.access_token);
    let encryptedRefreshToken = localCipher.encrypt(
      oThis.decryptedEncryptionSalt,
      oThis.appleOAuthDetails.refresh_token
    );
