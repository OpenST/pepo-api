...
class TwitterLogin extends LoginConnectBase {
  ...
  async _insertIntoSocialUsers() {
    ...
    if (CommonValidators.validateNonEmptyObject(oThis.twitterUserObj)) {
      ...
      Object.assign(twitterData, updateDataResponse.defaultUpdatedAttributes);
      Object.assign(oThis.twitterUserObj, new TwitterUserModel().formatDbData(twitterData));
    } else {
      let insertResponse = await new TwitterUserModel().insert(twitterData).fire();

      twitterData.id = insertResponse.insertId;
      Object.assign(twitterData, insertResponse.defaultUpdatedAttributes);

      oThis.twitterUserObj = new TwitterUserModel().formatDbData(twitterData);
    }
