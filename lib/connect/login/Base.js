...
class LoginConnectBase {
  ...
  async perform() {
    ...
    promisesArray.push(oThis._updateUsers());
  ...
  async _insertUserUniqueIdentifier() {
    ...
    if (oThis.isNewSocialConnect && socialEmail) {
      await new UserUniqueIdentifierModel().insertUserEmail(oThis.userId, socialEmail).catch(async function(err) {
        if (
          UserUniqueIdentifierModel.isDuplicateIndexViolation(UserUniqueIdentifierModel.userIdEmailUniqueIndexName, err)
        ) {
          logger.error('Duplicate Index violation for email and userId ', socialEmail, oThis.userId);
        } else {
          // Insert failed due to some other reason.
          // Send error email from here.
          const errorObject = responseHelper.error({
            internal_error_identifier: 'l_c_l_b_1',
  ...
  /**
   * Update users for login property.
   *
   * @returns {Promise<void>}
   * @private
   */
  async _updateUsers() {
    ...
    const propertyVal = userConstants.invertedProperties[oThis._userLoginPropertiesToSet()];
