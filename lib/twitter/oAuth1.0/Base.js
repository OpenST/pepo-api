...
class Base {
  ...
  _parseJsonResponse(jSonResponse) {
    ...
    if (jSonResponse.response.status == 200) {
      ...
      if (jSonResponse.response.status == 429) {
        return responseHelper.error({
          internal_error_identifier: 'twitterRateLimit',
          api_error_identifier: 'something_went_wrong',
          debug_options: { headers: jSonResponse.response.headers }
        });
      }

      if (jSonResponse.response.status == 401) {
        return responseHelper.error({
          internal_error_identifier: 'twitterUnauthorized',
          api_error_identifier: 'something_went_wrong',
          debug_options: { headers: jSonResponse.response.headers }
        });
      }
