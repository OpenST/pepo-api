...
class Base {
  ...
  async _parseJsonResponse(jSonResponse) {
    ...
    } else if (parsedResponse.errors[0].code == 187) {
      return responseHelper.error({
        internal_error_identifier: 'l_t_oa_a_1',
        api_error_identifier: 'duplicate_tweet',
        debug_options: { failureReason: jSonResponse }
      });
    } else if (parsedResponse.errors[0].code == 89) {
      return responseHelper.error({
        internal_error_identifier: 'l_t_oa_a_2',
        api_error_identifier: 'twitter_unauthorized',
        debug_options: { failureReason: {} }
      });
    } else if (
      jSonResponse.response.status === 304 ||
      jSonResponse.response.status === 429 ||
      jSonResponse.response.status === 403
    ) {
      logger.error('Error :::', parsedResponse);
