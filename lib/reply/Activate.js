...
class ReplyVideoPostTransaction {
  ...
  async perform() {
    ...
    logger.debug('CALLING _enqueueReplyNotification');
    await oThis._enqueueReplyNotification();
    logger.debug('DONE _enqueueReplyNotification');
  ...
  async _enqueueReplyNotification() {
    ...
    const pepoAmountInWei = await oThis._getPepoAmountInWei();
  ...
  /**
   * Get Pepo Amount in Wei
   *
   * @returns {Promise<void>}
   * @private
   */
  async _getPepoAmountInWei() {
    const oThis = this;

    const transactionId = oThis.replyDetail.transactionId;

    if (!transactionId) {
      return 0;
    }

    const cacheResponse = await new TransactionByIdsCache({ ids: [transactionId] }).fetch();

    if (cacheResponse.isFailure()) {
      return Promise.reject(cacheResponse);
    }

    const transactionObj = cacheResponse.data[transactionId];

    if (!CommonValidators.validateNonEmptyObject(transactionObj)) {
      return responseHelper.error({
        internal_error_identifier: 'l_r_a_gpaiw_1',
        api_error_identifier: 'something_went_wrong',
        debug_options: oThis.replyDetail
      });
    }
