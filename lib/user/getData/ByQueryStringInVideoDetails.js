...
class GetDataByQueryStringInVideoDetails {
  ...
  async _getUserIds() {
    ...
    const videoDetailsResp = await new VideoDetailsModel()
      .select('*')
  ...
  async fetchUserVideoDetails() {
    ...
    const videoDetailsResponse = await new VideoDetailsModel()
  ...
  finalResponse() {
    ...
    console.log('------oThis.userIdToFinalMap----------', oThis.userIdToFinalMap);
  ...
  printResponse() {
    ...
    console.log('userName, email, twitterHandle, inviteCodeUsed, isApprovedCreator, videoCount, pepoSpent');
