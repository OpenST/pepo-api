...
class GetUserProfile {
  ...
  async _fetchVideoDetails() {
    ...
    for (let videoId in oThis.videoDetails) {
      let video = oThis.videoDetails[videoId];
      oThis.descriptionIds.push(video.descriptionId);
    }

    oThis.allTextIds.concat(oThis.descriptionIds);
  }
  ...
  async _fetchTagAndLinkDetails() {
    ...
    for (let textId in oThis.textData) {
      ...
      if (oThis.textData[textId].kind === textConstants.invertedKinds[textConstants.videoDescriptionKind]) {
    ...
    oThis.allLinkIds.concat(allLinkIds);
  ...
  _fetchBioAndVideoDescription() {
    ...
    for (let textId in oThis.textData) {
      ...
      if (oThis.textData[textId].kind === textConstants.invertedKinds[textConstants.videoDescriptionKind]) {
        ...
        if (tagIds) {
          for (let i = 0; i < tagIds.length; i++) {
            ...
            videoDescription.includes[tagName] = {
              kind: 'tags',
              id: tagDetail.id
            };
          }
      ...
      } else if (oThis.textData[textId].kind === textConstants.invertedKinds[textConstants.bioKind]) {
        let tagIds = JSON.parse(oThis.textData[textId].tagIds);
        if (tagIds) {
          for (let i = 0; i < tagIds.length; i++) {
            let tagId = tagIds[i],
      ...
      oThis.videoDescription = videoDescription;
    }
  }
  ...
  async _fetchPendingTransactions() {
      ...
      for (let videoId in pendingTransactionsByVideoIds) {
        ...
        if (pendingTransactionsByVideoIdArray.length > 0) {
          ...
          for (let index = 0; index < pendingTransactionsByVideoIdArray.length; index++) {
            ...
            oThis.videoRspForPendingTransaction[videoId].totalAmount += Number(videoEntity.amount);
    ...
    let index = oThis.userIds.indexOf(oThis.currentUserId);
    let userIdsToCheck = oThis.userIds;
      ...
      for (let toUserId in pendingTransactionsByToUserIds) {
        ...
        if (pendingTransactionsByToUserIdArray.length > 0) {
          ...
          for (let index = 0; index < pendingTransactionsByToUserIdArray.length; index++) {
            ...
            oThis.userRspForPendingTransaction[toUserId].totalAmount += Number(userEntity.amount);
    ...
    for (let userId in oThis.userRspForPendingTransaction) {
      if (oThis.userStatMap[userId] && CommonValidators.validateNonEmptyObject(oThis.userStatMap[userId])) {
        oThis.userStatMap[userId].totalAmountRaised = (
          Number(oThis.userStatMap[userId].totalAmountRaised) + oThis.userRspForPendingTransaction[userId].totalAmount
        ).toString();
        ...
        oThis.userStatMap[userId].totalContributedBy = oThis.userRspForPendingTransaction[userId].totalTransaction;
  ...
  _updateVideoDetails() {
    ...
    for (let videoId in oThis.videoRspForPendingTransaction) {
      if (oThis.videoDetails[videoId] && CommonValidators.validateNonEmptyObject(oThis.videoDetails[videoId])) {
        oThis.videoDetails[videoId].totalTransactions += oThis.videoRspForPendingTransaction[videoId].totalTransaction;
        oThis.videoDetails[videoId].totalAmount = (
          Number(oThis.videoDetails[videoId].totalAmount) + oThis.videoRspForPendingTransaction[videoId].totalAmount
        ).toString();
        if (oThis.videoDetails[videoId].updatedAt < oThis.videoRspForPendingTransaction[videoId].uts) {
          oThis.videoDetails[videoId].updatedAt = oThis.videoRspForPendingTransaction[videoId].uts;
        }
        if (!CommonValidators.validateNonEmptyObject(oThis.currentUserVideoContributionsMap[videoId])) {
          oThis.videoDetails[videoId].totalContributedBy += 1;
        }
      }
    }
  }
      ...
      if (oThis.currentUserUserContributionsMap[userId]) {
        if (CommonValidators.validateNonEmptyObject(oThis.currentUserUserContributionsMap[userId])) {
          oThis.currentUserUserContributionsMap[userId].totalAmount = (
            Number(oThis.currentUserUserContributionsMap[userId].totalAmount) +
      ...
      if (oThis.currentUserVideoContributionsMap[videoId]) {
        if (CommonValidators.validateNonEmptyObject(oThis.currentUserVideoContributionsMap[videoId])) {
          oThis.currentUserVideoContributionsMap[videoId].totalAmount = (
            Number(oThis.currentUserVideoContributionsMap[videoId].totalAmount) +
