...
class GetUserProfile {
  ...
  _getUserProfileAllowedActions(userId) {
    ...
    if (userId === oThis.currentUserId) {
      ...
      if (oThis.blockedByUserInfo.hasBlocked.indexOf(userId) > -1) {
        oThis.userProfileAllowedActions[userId]['canBlock'] = 0;
  ...
  forwardVideo(userId, userObj) {
    ...
    if (
      oThis.blockedByUserInfo.hasBlocked.indexOf(userObj.id) > -1 ||
      oThis.blockedByUserInfo.blockedBy.indexOf(userObj.id) > -1
    ) {
      return false;
    }
