...
class GetUserProfile {
  ...
  async _fetchVideoDetails() {
    ...
    for (let videoId in oThis.videoDetails) {
      let videoDetail = oThis.videoDetails[videoId];
      ...
      if (
        !userObj ||
        userObj.status === userConstants.inActiveStatus ||
        (oThis.currentUserId != videoDetail.creatorUserId && !UserModel.isUserApprovedCreator(userObj))
      ) {
        ...
        delete oThis.videoDetails[videoId];
        continue;
      }
  ...
  async _fetchElementData(userId, userObj, kind, data) {
    ...
    switch (kind) {
      ...
      case userProfileElementConst.coverVideoIdKind:
        ...
        if (oThis.forwardVideo(userId, userObj)) {
          oThis.userProfilesMap[userId][kind] = data;
  ...
  forwardVideo(userId, userObj) {
    ...
    return !!(oThis.isAdmin || oThis.currentUserId == userId || UserModel.isUserApprovedCreator(userObj));
  }
}
