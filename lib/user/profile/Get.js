...
class GetUserProfile {
  ...
  _getUserProfileAllowedActions(userId) {
    ...
    oThis.userProfileAllowedActions[userId]['blockedRelations'] = 0;
    if (userId === oThis.currentUserId) {
      ...
      if (oThis.blockedByUserList.includes(userId)) {
        oThis.userProfileAllowedActions[userId]['blockedRelations'] = 1;
      }
    }
  }
    ...
    for (let videoId in oThis.videoRspForPendingTransaction) {
      if (oThis.videoDetailsMap[videoId] && CommonValidators.validateNonEmptyObject(oThis.videoDetailsMap[videoId])) {
        ...
        oThis.videoDetailsMap[videoId].totalAmount = (
          Number(oThis.videoDetailsMap[videoId].totalAmount) + oThis.videoRspForPendingTransaction[videoId].totalAmount
        ).toString();
  ...
  forwardVideo(userId, userObj) {
    ...
    if (oThis.blockedByUserList.includes(userObj.id)) {
      return false;
    }
