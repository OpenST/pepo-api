...
class GetUserProfile {
   ...
   * @constructor
   *
   * todo: currentUserId can be blank here. Handle the scenario.
   * @param params
  ...
  async _fetchUsers() {
    ...
    //todo: fetch only for concerned users??? and not all???
  ...
  async _fetchTagAndLinkDetails() {
    ...
    if (oThis.allTextIds.length > 0) {
      const cacheRsp = await new TextsByIdCache({ ids: oThis.allTextIds }).fetch();
      if (cacheRsp.isFailure()) {
        return Promise.reject(cacheRsp);
      }
      ...
      const includesCacheRsp = await new TextIncludesByIdsCache({ ids: oThis.allTextIds }).fetch();
  ...
  _fetchBioAndVideoDescription() {
    ...
    for (const textId in oThis.textData) {
      const userId = oThis.textIdToUserIdMap[textId],
        textIncludes = oThis.includesData[textId],
        tagIds = [],
        linkIds = [];
      ...
      for (let ind = 0; ind < textIncludes.length; ind++) {
        ...
        if (entity[0] == textIncludeConstants.tagEntityKindShort) {
          ...
          linkIds.push(+entity[1]); // Inserting only video description links, bio links are ignored
        }
      }

      if (oThis.textData[textId].kind === textConstants.videoDescriptionKind) {
        const videoDescription = oThis._formatTags(tagIds, textId);
  ...
  async _fetchTags(tagIds) {
    ...
    const cacheRsp = await new TagByIdCache({ ids: tagIds }).fetch();
