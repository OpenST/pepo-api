...
class GetUserProfile {
  ...
  _getUserProfileAllowedActions(userId) {
    ...
    if (userId === oThis.currentUserId) {
      ...
      if (oThis.mutedUsersMap && oThis.mutedUsersMap[userId]) {
        oThis.userProfileAllowedActions[userId]['canMute'] = 0;
  ...
  async _fetchMuteUsers() {
    const oThis = this;
    logger.debug('======= Fetching UserMuteByUser1IdsCache for user id: ', oThis.currentUserId);

    let userId = oThis.currentUserId;

    console.log('-----ADMIN----', oThis.isAdmin);
    if (oThis.isAdmin) {
      console.log('---------sdsdfs');
      userId = 0;
    }
    const userMuteByUser1IdsCacheResp = await new UserMuteByUser1IdsCache({ user1Ids: [userId] }).fetch();

    console.log('---userMuteByUser1IdsCacheResp---', userMuteByUser1IdsCacheResp);
    ...
    logger.debug('======= userMuteByUser1IdsCacheRespData ================: ', userMuteByUser1IdsCacheRespData[userId]);
    oThis.mutedUsersMap = userMuteByUser1IdsCacheRespData[userId];
  }
