...
class UpdateStats {
  ...
  async perform() {
    ...
    if (oThis.videoId) {
      promiseArray.push(oThis._updateVideoContributors());
    ...
    logger.log('Total time : ', currentTime - Date.now());
  }
  ...
  async _updateVideoDetails() {
    ...
    const updateResponse = await new VideoDetailsModel().updateByVideoId(oThis.videoId, oThis.totalAmount);

    // Create video details if row doesn't exist.
    if (updateResponse.changedRows === 0) {
      const insertResponse = await new VideoDetailsModel().insertIntoVideoDetails(
        oThis.toUserId,
        oThis.videoId,
        oThis.totalAmount
      );
  ...
  async _updateUserStats() {
    ...
    const updateFromUserIdRsp = await new UserStatModel().updateUserStat(oThis.fromUserId, 1, 0, 0);

    // Update user stat for toUserId.
    const updateToUserIdRsp = await new UserStatModel().updateUserStat(oThis.toUserId, 0, 1, oThis.totalAmount);
  ...
  async _createUserStats() {
    ...
    await new UserStatModel().createUserStat(oThis.fromUserId, 1, 0, 0).catch(async function(error) {
      // Update user stat for fromUserId.
      const updateFromUserIdRsp = await new UserStatModel().updateUserStat(oThis.fromUserId, 1, 0, 0);
    });

    // Create user stat for toUserId.
    const createToUserIdRsp = await new UserStatModel()
      .createUserStat(oThis.toUserId, 0, 1, oThis.totalAmount)
  ...
  async _updateTwitterUserConnection() {
    ...
    if (
      !twitterUserCacheRsp.data[oThis.fromUserId] ||
      !twitterUserCacheRsp.data[oThis.fromUserId].id ||
      !twitterUserCacheRsp.data[oThis.toUserId] ||
      !twitterUserCacheRsp.data[oThis.toUserId].id
    ) {
