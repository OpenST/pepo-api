...
class TwitterFriendSync {
  ...
  async _start() {
    ...
    let resp = await new FriendsTwitterRequestClass().getIds(reqParams).catch(function(e) {
      return responseHelper.error({
        internal_error_identifier: 'l_tws_s_1',
    ...
    if (resp.isFailure()) {
      await oThis._reEnqueue();
      return resp;
    }

    console.log(`getFriendsIdsData======= currentIterator=>${currentIterator} ====`, resp.data);
  ...
  async _fetchUserFromTwitter() {
    ...
    let maxIteratorCount = Math.ceil(nonExistingTwitterIdsSet.length / twitterLookupSize);

    for (let i = 0; i < maxIteratorCount; i++) {
      let twitterIdsToLookup = nonExistingTwitterIdsSet.slice(i * 100, (i + 1) * 100);

      let bulkInsertVal = [];
      await new UsersTwitterRequestClass().lookup({ twitterIds: twitterIdsToLookup }).then(function(userLookupResp) {
