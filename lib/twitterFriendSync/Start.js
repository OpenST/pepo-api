...
class TwitterFriendSync {
  ...
  async _start() {
    ...
    let twitterUserMapByTwitterId = await oThis._getTwitterUserMap(currentIterator, resp.data.ids);
  ...
  async _reEnqueue(resetTime) {
    ...
    if (resetTime) {
      let publishAfter = resetTime * 1000 - Date.now();
  ...
  async _clearTwitterUserConnectionCache() {
    ...
    await new TwitterUserConnectionByUser1PaginationCache({
