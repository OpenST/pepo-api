...
// Todo: rejection due to rate limit else error mail with enqueue params
//Todo: sigint handle
  ...
  async _populateTwitterUserConnection(twitterUserMapByTwitterId) {
    ...
    for (let twitterId in twitterUserMapByTwitterId) {
      ...
      if (twitterUserObj.userId) {
        registeredUserIdsMap[twitterUserObj.userId] = twitterUserObj.twitterId;
        continue;
      }

      let val = [oThis.twitterId, twitterUserObj.twitterId, 0];
    ...
    for (let j = 0; j < maxIteratorCount; j++) {
      ...
      for (let i = 0; i < oThis.parallelThreads; i++) {
        ...
        let promise = new TwitterUserConnectionModel()
          .insertMultiple(['twitter_user_id1', 'twitter_user_id2', 'properties'], insertData, { withIgnore: true })
          .fire();
    ...
    await oThis._populateRegisterTwitterUserConnection(registeredUserIdsMap, twitterUserMapByTwitterId);
    logger.log(`end: _populateTwitterUserConnection()`);
  }

  async _populateRegisterTwitterUserConnection(registeredUserIdsMap, twitterUserMapByTwitterId) {
    const oThis = this;
    ...
    for (let j = 0; j < maxIteratorCount; j++) {
      ...
      for (let i = 0; i < oThis.parallelThreads; i++) {
        ...
        let promise = new TwitterUserByTwitterIdsCache({ contributedByIds: userIds })
          .fetch()
          .then(async function(cacheResp) {
            ...
            let val = null;
            for (let contributedById in cacheResp) {
              let userContributedByobj = cacheResp[contributedById];
              let twitterId = registeredUserIdsMap[contributedById];

              if (userContributedByobj.id) {
                val = [oThis.twitterId, twitterId, contributedTopropertyVal];
              } else {
                val = [oThis.twitterId, twitterId, twitterUser2RegisteredVal];
              }
            ...
            await new TwitterUserConnectionModel()
              .insertMultiple(['twitter_user_id1', 'twitter_user_id2', 'properties'], bulkInsertVal)
