const rootPrefix = '../..',
  ...
  UserContributedByUIdsAndCBUIdCache = require(rootPrefix +
    '/lib/cacheManagement/multi/UserContributedByUserIdsAndContributedByUserId'),
  ...
  async _populateRegisterTwitterUserConnection(registeredUserIdsMap, twitterUserMapByTwitterId) {
    ...
    for (let j = 0; j < maxIteratorCount; j++) {
      ...
      for (let i = 0; i < oThis.parallelThreads; i++) {
        ...
        let promise = new UserContributedByUIdsAndCBUIdCache({ userIds: userIds, contributedById: oThis.currentUserId })
          .fetch()
          .then(async function(cacheResp) {
            ...
            let val = null;
            for (let contributedToId in cacheResp) {
              let userContributedByobj = cacheResp[contributedToId];
              let twitterId = registeredUserIdsMap[contributedToId];
