...
class SaveRecording {
  ...
  constructor(params) {
    ...
    oThis.recordingFiles = params.recordingFiles || [];
  ...
  async _asyncPerform() {
    ...
    if (oThis.recordingFiles.length === 0) {
      await oThis._fetchRecordingURL();
    } else {
      await oThis._validateAccessToken();
    }
  ...
  async _validateAccessToken() {
    ...
    // Converted to string
    oThis.accessToken = oThis.accessToken ? oThis.accessToken.toString() : oThis.accessToken;
    if (!CommonValidators.validateNonBlankString(oThis.accessToken)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_z_sr_v_2',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
  ...
  _download(recordingFile) {
    ...
    return new Promise(async function(onResolve, onReject) {
      ...
      const options = {
        uri: oThis.accessToken
          ? `${recordingFile.download_url}?access_token=${oThis.accessToken}`
