...
class SaveRecording {
  ...
  async _fetchRecordingURL() {
    ...
    const response = await ZoomLib.getRecording(oThis.zoomMeetingId).catch(async (e) => {
  ...
  async _downloadFiles() {
    ...
    await Promise.all(downloadPromises).catch(async (e) => {
      logger.error(
  ...
  async _uploadToS3() {
    ...
    return Promise.all(putObjectPromises).catch(async (e) => {
      const errorObject = responseHelper.error({
  ...
  async _deleteRecordingsInZoom() {
    ...
    for (let i = 0; i < oThis.filesForDeletion.length; i++) {
      logger.debug(`Deleting file ${oThis.filesForDeletion[i].download_url} having id ${oThis.filesForDeletion[i].id}`);
      deletedRecordingPromises.push(ZoomLib.deleteRecordingFile(oThis.zoomMeetingId, oThis.filesForDeletion[i].id));
    }

    await Promise.all(deletedRecordingPromises).catch((e) => {
      logger.error(`Failed deleting recording files for zoom meeting ID ${oThis.zoomMeetingId} , error: ${e.message}`);

      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_z_sr_df_2',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
  ...
  filesToBeDeleted(recordingFiles) {
    ...
    return recordingFiles.filter((file) => Object.prototype.hasOwnProperty.call(file, 'id'));
  }
