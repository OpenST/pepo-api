...
class SaveRecording {
  ...
  async _downloadFiles() {
    ...
    oThis.accessToken = jwtHelper.fetchToken();
  ...
  _download(recordingFile) {
    ...
    return new Promise(async function(onResolve, onReject) {
      ...
      const options = {
        uri: `${recordingFile.download_url}?access_token=${oThis.accessToken}`
      };
  ...
  async recordingLocalFilePath(fileType, recordingType) {
    ...
    if (extension === 'mp4') {
      oThis.localMP4FilePaths.push(filePath);
    }
  ...
  async _getFileTypeAndExtension(fileType, recordingType) {
    ...
    switch (identifier) {
      ...
      case zoomRecordingConstants.sharedScreenWithGalleryViewType: {
        return {
          customRecordingType: recordingType,
          extension: 'mp4'
        };
  ...
  async _validateDownload() {
    ...
    for (let i = 0; i < oThis.localMP4FilePaths.length; i++) {
      const localMP4FilePath = oThis.localMP4FilePaths[i];
      const stats = fs.statSync(localMP4FilePath);
      logger.debug(`File size is ${stats.size} for file ${localMP4FilePath}`);

      if (stats.size < 60000) {
        //60kb

        //Note: Always Reject and stop processing Else file will be deleted by delete method
