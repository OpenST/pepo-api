...
class VideosPerformance {
  ...
  async _fetchVideoDetails(limit, offset) {
    ...
    let dbRows = await new VideoDetailModel()
      .select('*')
    ...
    for (let index = 0; index < oThis.totalRows; index++) {
      let videoDetail = dbRows[index];
      videoIds.push(videoDetail.video_id);
      creatorUserIds.push(videoDetail.creator_user_id);
    }

    let userMap = await new UserModel().fetchByIds(creatorUserIds);
    let videoContributorRows = await new VideoContributorModel()
      .select('*')
    ...
    let videoUrlMap = await oThis._getVideoUrl(videoIds);

    let videoTagsMap = await oThis._getTagsForVideo(videoIds);

    let videoContributorMap = {};
    for (let index = 0; index < videoContributorRows.length; index++) {
      let row = videoContributorRows[index];
    ...
    for (let index = 0; index < oThis.totalRows; index++) {
      ...
      oThis.videoData.last_pepo_coin_recieved_on = videoContributorMap[videoId]
        ? new Date(videoContributorMap[videoId].updated_at * 1000)
        : '';
  ...
  async _getTagsForVideo(videoIds) {
    ...
    let videoTagRows = await new VideoTagsModel()
    ...
    for (let index = 0; index < tagRows.length; index++) {
      let row = tagRows[index];
      if (tagVideosMap[row.id]) {
        for (let j = 0; j < tagVideosMap[row.id].length; j++) {
          let vid = tagVideosMap[row.id][j];
  ...
  async _processOutput() {
    ...
    for (let index = 0; index < oThis.videoDetails.length; index++) {
      let vd = oThis.videoDetails[index];
      csvContent +=
        vd.creator_name +
        ',' +
