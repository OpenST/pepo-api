...
class UserData {
  ...
  async fetchUserDetails() {
    ...
    while (isMoreDataPresent) {
      ...
      if (userDbRows.length === 0) {
        ...
        page++;
        offset = page * limit;
        let batchedDataWithInviteCode = await oThis.fetchInviteCodeDetails(batchdata),
        ...
        Object.assign(oThis.finalData, batchedDataWithTopupInfo);
        //isMoreDataPresent = false;
      }
    }
  }
  ...
  async fetchInviteCodeDetails(batchdata) {
    ...
    let userIds = Object.keys(batchdata);
    ...
    let inviteCodeIds = Object.keys(inviterCodeIdToUserIdMap);
    ...
    if (inviteCodeIds.length !== 0) {
      ...
      for (let index = 0; index < inviteCodeByIdsRows.length; index++) {
        ...
        if (inviteCodeRow.kind == 2) {
          let userIdsForThisCode = inviterCodeIdToUserIdMap[inviteCodeRow.id];
          for (let i = 0; i < userIdsForThisCode.length; i++) {
            batchdata[userIdsForThisCode[i]].custom_invite_code = inviteCodeRow.code;
          }
        } else if (inviteCodeRow.user_id) {
          let userIdsForThisCode = inviterCodeIdToUserIdMap[inviteCodeRow.id];
          for (let i = 0; i < userIdsForThisCode.length; i++) {
            batchdata[userIdsForThisCode[i]].inviter_user_id = inviteCodeRow.user_id;
            //Fetch user id;
            console.log('---inviteCodeRow---', inviteCodeRow);
            batchdata[userIdsForThisCode[i]].inviter_user_name = await oThis.fetchUserName(inviteCodeRow.user_id);
          }
        }
      }
    }
  ...
  async fetchUserStats(batchedData) {
    ...
    let userIds = Object.keys(batchedData);
  ...
  printFinalResponse() {
    ...
    for (const userId in oThis.finalData) {
      ...
      console.log(
        ...
        }, ${individualUserData.last_topup_date}, ${basicHelper.convertWeiToNormal(
