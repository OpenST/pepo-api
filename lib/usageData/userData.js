const rootPrefix = '.', // TODO: Correct this.
...
class UserData {
  ...
  async perform() {
    ...
    for (let index = 0; index < oThis.userIds.length; index += batchSize) {
      const tempArray = oThis.userIds.slice(index, index + batchSize);
      console.log('tempArray=====', tempArray);
      const promisesArray = [
        oThis.fetchInviteCodeDetails(tempArray),
        oThis.fetchUserVideoDetails(tempArray),
        oThis.fetchUserStats(tempArray),
        oThis.fetchTopUpInformation(tempArray)
      ];
   ...
   *
   * @sets oThis.inviterCodeIds
   *
   * @returns {Promise<void>}
   */
  async fetchInviteCodeDetails(userIds) {
    const oThis = this;
    ...
    for (let index = 0; index < inviteCodeByUserIdsRows.length; index++) {
      ...
      console.log('==inviteCodeRow====', inviteCodeRow);

      userIdToInviteCodeDetails[inviteCodeRow.user_id] = {
        is_creator: inviteCodeRow.invite_limit <= 0 ? 1 : 0,
    ...
    if (inviterCodeIds.length === 0) {
      for (const userId in oThis.userIdToUserDetailsMap) {
        //console.log('oThis.userIdToUserDetailsMap--userId---', oThis.userIdToUserDetailsMap[userId]);
        console.log('userIdToInviteCodeDetails----userId--', userId, userIdToInviteCodeDetails[userId]);
        if (userIdToInviteCodeDetails[userId] && userIdToInviteCodeDetails[userId].is_creator) {
   ...
   * @returns {Promise<void>}
   */
  async fetchUserVideoDetails(userIds) {
    const oThis = this;

    const videoDetailRows = await new VideoDetailModel()
      .select(
        'creator_user_id, created_at, MAX(created_at) AS firstVideoCreation, MIN(created_at) AS lastVideoCreation, COUNT(1) AS count'
      )
    ...
    for (let index = 0; index < videoDetailRows.length; index++) {
      ...
      console.log('videoDetailRow----', videoDetailRow);

      console.log(
        'videoDetailRow.firstVideoCreation----====, basicHelper.timeStampInMinutesToDateTillSeconds(videoDetailRow.firstVideoCreation) ',
        videoDetailRow.firstVideoCreation,
        basicHelper.timeStampInMinutesToDateTillSeconds(videoDetailRow.firstVideoCreation)
      );
      creatorUserIdToVideoDetails[videoDetailRow.creator_user_id] = {
    ...
    console.log('creatorUserIdToVideoDetails[userId]-----', creatorUserIdToVideoDetails);
   ...
   * @returns {Promise<void>}
   */
  async fetchTopUpInformation(userIds) {
    const oThis = this;
  ...
  printFinalResponse() {
    ...
    logger.log(
      'user_id, sign_up_date, name, email, is_creator, inviter_user_name, custom_invite_code, total_videos, first_video_created_on, last_video_created_on, topup_amount, last_topup_date, amount_raised, amount_spent'
    );

    for (const userId in oThis.userIdToUserDetailsMap) {
      const individualUserData = oThis.userIdToUserDetailsMap[userId];

      logger.log(
        `${userId}, ${individualUserData.sign_up_date}, ${individualUserData.name}, ${individualUserData.email}, ${
          individualUserData.is_creator
        }, ${individualUserData.inviter_user_name}, ${individualUserData.custom_invite_code}, ${
          individualUserData.total_videos
        }, ${individualUserData.first_video_created_on}, ${individualUserData.last_video_created_on}, ${
          individualUserData.topup_amount
        }, ${individualUserData.last_topup_date}, ${individualUserData.amount_raised}, ${
          individualUserData.amount_spent
        }`
      );
    }
  }
