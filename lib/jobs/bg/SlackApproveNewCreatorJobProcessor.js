...
class SlackApproveNewCreatorJobProcessor {
  ...
  _generateMessage() {
    ...
    let message = `---------------------------------------------------------\n
      ...
      *User's Full Name:*: ${oThis.userData.name}\n
    ...
    for (let videoId in oThis.videoTextMap) {
      blocks.push(slackConstants.addVideoLinkSection(oThis.videoTextMap[videoId], videoId));
      blocks.push(slackConstants.addDividerSection);
    }

    blocks.push(slackConstants.addApproveBlockSection(oThis.userId));
    blocks.push(slackConstants.addTextSection(separator));

    // console.log("The blocks is : ",JSON.parse(blocks.toString()));
    return blocks;
  }
