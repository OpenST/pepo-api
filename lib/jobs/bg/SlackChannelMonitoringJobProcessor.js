...
class SlackChannelMonitoringJobProcessor {
  ...
  async _fetchUser() {
    ...
    if (oThis.source === slackConstants.adminSource) {
      ...
      cacheResponse = await new SecureUserCache({ id: oThis.sourceId }).fetch();
    }
    ...
    oThis.sourceData = cacheResponse.data;

    if (!CommonValidators.validateNonEmptyObject(oThis.sourceData)) {
  ...
  _generateMessage() {
    ...
    if (oThis.action === slackConstants.channelCreated) {
      message = `*Hi, Pepo Community created: *\n
        *Community Type*: New\n
        *Status*: ${status}\n
        *Created By*: ${oThis.sourceData.name}\n
      ...
      message = `*Hi, Pepo Community has been updated: *\n
        ...
        *Status*: ${status}\n
        *Updated By*: ${oThis.sourceData.name}\n
        *Name*: ${oThis.channelDetails.name}\n
        *Link*: ${communityUrl}\n
        *Admin*: ${oThis.channelAdmins.join(', ')}\n
        *Tagline*: ${oThis.channelTagline}\n
