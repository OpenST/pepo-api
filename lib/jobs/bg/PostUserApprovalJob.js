const rootPrefix = '../../..',
  ...
  AddPostToGraphLib = require(rootPrefix + '/lib/video/AddPostToGraph'),
...
class PostUserApprovalJob {
  ...
  async perform() {
    ...
    for (let index = 0; index < videoIds.length; index++) {
      ...
      const promiseArray = [oThis._addPostToGraph(videoId), oThis._updateTagAndVideoTag(videoId)];
  ...
  async _publishFanVideo() {
    ...
    if (latestVideoId) {
      ...
      promises.push(oThis._publishAtMentionNotifications(latestVideoId), oThis._addFeed(latestVideoId));
  ...
  async _addFeed(videoId) {
    ...
    await FeedModel.flushCache({ id: feedId });
  }
   ...
   * @private
   */
  async _addPostToGraph(videoId) {
    const oThis = this;
    const videoDetailObj = oThis.userVideoIdsMap.videoDetails[videoId];

    const params = {
      creatorUserId: oThis.userId,
      videoId: videoId,
      createdAt: videoDetailObj.createdAt
    };

    await new AddPostToGraphLib(params).perform();
  }
