...
class SlackCommunityMonitoringJobProcessor {
  ...
  async perform() {
    ...
    await oThis._fetchChannelDetails(),
      await oThis._fetchTagLine(),
      await oThis._fetchChannelTagIds(),
      await oThis._fetchChannelDescription(),
  ...
  async _fetchTagLine() {
    ...
    const textCacheResponse = await new TextsByIdCache({ ids: [oThis.channelDetails.taglineId] }).fetch();
    if (textCacheResponse.isFailure()) {
      return Promise.reject(textCacheResponse);
    }

    const channelTaglineObject = textCacheResponse.data[oThis.channelDetails.taglineId];
  ...
  async _fetchChannelTagIds() {
    ...
    oThis.channelTags = channelTagByChannelIdsCacheCacheData[channelId];
  }

  /**
   * Fetch channel description.
   *
   * @sets oThis.channelDescription
   *
   * @returns {Promise<void>}
   * @private
   */
  async _fetchChannelDescription() {
    const oThis = this;

    // TODO: Add logic to fetch channel description
    oThis.channelDescription = '';
  }
