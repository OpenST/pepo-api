...
class SlackLiveEventMonitoringJobProcessor {
   ...
   * @param {String} params.errorGoingLive
   *
   * @constructor
   */
  constructor(params) {
    ...
    oThis.errorGoingLive = params.errorGoingLive || null;
  ...
  async _fetchChannelLiveMeeting() {
    ...
    if (!channelMeetingIdsResp.data[oThis.channelId].liveMeetingId) {
      return Promise.reject(
        responseHelper.paramValidationError({
  ...
  _generateMessage() {
    ...
    let message = `*Hi, Pepo Community is now live : *\n
    ...
    if (oThis.errorGoingLive) {
      message = `*Hi, Pepo Community is not able to go live : *\n
        *Community*: ${oThis.channelDetails.name}\n
        *Meeting Url*: ${meetingUrl}\n
        *Error*: ${oThis.errorGoingLive}\n`;
    }

    message += `*Community Admin*: ${oThis.userData.name}\n
  ...
  async _sendSlackMessage() {
    ...
    console.log('slackMessageParams: ', slackMessageParams);
