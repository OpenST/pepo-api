...
/**
 * Class to populate channel data sheet in Google Sheets.
 *
 * @class ChannelData
 */
class ChannelData {
  /**
   * Constructor to populate channel data sheet in Google Sheets.
   *
   * @param {object} params
   *
   * @constructor
  ...
  async fetchVideoDetails() {
    ...
    while (true) {
      ...
      const dbRows = await new VideoDetailModel()
        ...
        .where({
          video_id: videoIdBatch,
          status: videoDetailConstant.invertedStatuses[videoDetailConstant.activeStatus]
        })
        .fire();
      ...
      for (let i = 0; i < contributionDbRows.length; i++) {
        ...
        oThis.videoData[dbRow.video_id]['lastTransactionTime'] = dbRow.lastTransactionTime;
      }

      await oThis.fetchReplyDetails(videoIdBatch);
    }
  }
   ...
   * @returns {Promise<void>}
   */
  async fetchReplyDetails(videoIdBatch) {
    const oThis = this;
    ...
    while (true) {
      const replyDbRows = await new ReplyDetailModel()
        .select('parent_id, entity_id, total_transactions, created_at')
        .where(['status != ?', replyDetailConstant.invertedStatuses[replyDetailConstant.deletedStatus]])
      ...
      for (let i = 0; i < replyDbRows.length; i++) {
        ...
        if (
          !oThis.videoData[dbRow.parent_id]['lastReplyTime'] ||
          oThis.videoData[dbRow.parent_id]['lastReplyTime'] < dbRow.created_at
        ) {
          oThis.videoData[dbRow.parent_id]['lastReplyTime'] = dbRow.created_at;
        }

        oThis.videoData[dbRow.parent_id]['totalTransactions'] += dbRow.total_transactions;
        replyIdMap[dbRow.entity_id] = dbRow.parent_id;
        replyVideoIds.push(dbRow.entity_id);
      }

      if (replyVideoIds.length > 0) {
        const contributionReplyDbRows = await new VideoContributorModel()
          .select('video_id, max(created_at) as lastTransactionTime')
          .where({ video_id: replyVideoIds })
          .group_by('video_id')
          .fire();

        for (let i = 0; i < contributionReplyDbRows.length; i++) {
          const dbRow = contributionReplyDbRows[i];
          const vid = replyIdMap[dbRow.video_id];

          if (
            !oThis.videoData[vid]['lastTransactionTime'] ||
            oThis.videoData[vid]['lastTransactionTime'] < dbRow.lastTransactionTime
          ) {
            oThis.videoData[vid]['lastTransactionTime'] = dbRow.lastTransactionTime;
          }
        }
  ...
  async fetchChannelVideos(channelId) {
    ...
    while (true) {
      const dbRows = await new ChannelVideoModel()
        .select(['channel_id', 'video_id', 'created_at'])
        .where({
  ...
  async uploadData() {
    ...
    for (const videoId in oThis.videoData) {
      ...
      for (let i = 0; i < channelIds.length; i++) {
        ...
        if (
          videoDetail.lastReplyTime &&
          (!channelDetail.lastReplyTime || channelDetail.lastReplyTime < videoDetail.lastReplyTime)
        ) {
          channelDetail.lastReplyTime = videoDetail.lastReplyTime;
        }

        if (
          videoDetail.lastTransactionTime &&
          (!channelDetail.lastTransactionTime || channelDetail.lastTransactionTime < videoDetail.lastTransactionTime)
        ) {
          channelDetail.lastTransactionTime = videoDetail.lastTransactionTime;
        }

        if (videoDetail.totalHalfWatchedVideo) {
          channelDetail.totalHalfWatchedVideo = channelDetail.totalHalfWatchedVideo + videoDetail.totalHalfWatchedVideo;
        }

        if (videoDetail.totalHalfWatchedReply) {
          channelDetail.totalHalfWatchedReply = channelDetail.totalHalfWatchedReply + videoDetail.totalHalfWatchedReply;
        }

        if (
          videoDetail.lastHalfWatchedDateVideo &&
        ...
        if (
          videoDetail.lastHalfWatchedDateReply &&
