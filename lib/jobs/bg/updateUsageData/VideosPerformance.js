...
class VideosPerformance {
  ...
  async fetchVideoDetails(limit, offset) {
    ...
    for (let index = 0; index < oThis.totalRows; index++) {
      ...
      parentVideoIds.push(videoDetail.parent_id);
    }
    ...
    const parentIdToUniqueRepliesMap = await oThis.getUniqueRepliesOnVideos(parentVideoIds);
    ...
    for (let index = 0; index < oThis.totalRows; index++) {
      const videoDetail = dbRows[index],
        ...
        parentId = videoDetail.parent_id;
      ...
      oThis.videoData.unique_replies_on_video = parentIdToUniqueRepliesMap[parentId]
        ? parentIdToUniqueRepliesMap[parentId]
        : 0;
  ...
  /**
   * Get unique replies on videos tags.
   *
   * @param parentVideoIds
   *
   * @returns {Promise<void>}
   */
  async getUniqueRepliesOnVideos(parentVideoIds) {
    const dbRows = await new ReplyDetailModel()
      .select('parent_id, count(distinct(creator_user_id)) as unique_replies_on_video')
      .where(['parent_id IN (?)', parentVideoIds])
      .group_by('parent_id')
      .fire();

    const parentIdToUniqueRepliesMap = {};
    for (let index = 0; index < dbRows.length; index++) {
      let dbRow = dbRows[index];
      parentIdToUniqueRepliesMap[dbRow.parent_id] = dbRow.unique_replies_on_video;
    }

    return parentIdToUniqueRepliesMap;
  }
