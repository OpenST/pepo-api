...
class VideosPerformance {
  ...
  async fetchVideoDetails(limit, offset) {
    ...
    const parentIdToUniqueRepliesMap = await oThis.getUniqueRepliesOnVideos(videoIds);
    ...
    for (let index = 0; index < oThis.totalRows; index++) {
      ...
      oThis.videoData.unique_replies_on_video = parentIdToUniqueRepliesMap[videoId]
        ? parentIdToUniqueRepliesMap[videoId]
        : 0;
   ...
   * @returns {Promise<void>}
   */
  async getUniqueRepliesOnVideos(videoIds) {
