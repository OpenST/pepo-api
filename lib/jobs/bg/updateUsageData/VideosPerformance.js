...
class VideosPerformance {
  ...
  async fetchTransactionDetails() {
    ...
    while (true) {
      const whereClauseArray = ['status = ?', transactionConstants.invertedStatuses[transactionConstants.doneStatus]];

      const kindArray = [transactionConstants.invertedKinds[transactionConstants.userTransactionKind]];
      ...
      const transactionRows = await new TransactionModel()
        .select('from_user_id, amount, extra_data, created_at')
      ...
      for (let index = 0; index < transactionRows.length; index++) {
        ...
        let amount = basicHelper
          .convertToBigNumber(videoIdByTransactionData[videoId].pepoCoinsRecieved)
          .plus(basicHelper.convertToBigNumber(transactionRow.amount));

        videoIdByTransactionData[videoId].pepoCoinsRecieved = amount.toString(10);
  ...
  async fetchVideoDetails(limit, offset) {
    ...
    if (
      !oThis.allVideosButOnlyTimeIntervalData &&
