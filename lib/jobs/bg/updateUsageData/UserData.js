...
class UserData {
  ...
  async fetchTransactionDetails(batchedData) {
    ...
    const transactionRows = await new TransactionModel()
      ...
      .where(whereClauseArray)
      .where({ kind: [transactionConstants.userTransactionKind, transactionConstants.userTransactionOnReplyKind] })
      .group_by(['from_user_id'])
      .fire();
  ...
  processCsvDataForVideos() {
    ...
    for (let index = 0; index < parsedDataArray.length; index++) {
      ...
      if (Number(parsedDataRow.u_id) > 0) {
        ...
        oThis.userIdToVideoDetailsMap[parsedDataRow.u_id].total_videos_watched += +parsedDataRow.total_videos_watched;
    ...
    for (let index = 0; index < parsedDataArray.length; index++) {
      ...
      if (Number(parsedDataRow.u_id) > 0) {
        oThis.userIdToReplyDetailsMap[parsedDataRow.u_id] = oThis.userIdToReplyDetailsMap[parsedDataRow.u_id] || {
          total_replies_watched: 0,
          latestTimestamp: rowTimestamp
        };

        oThis.userIdToReplyDetailsMap[parsedDataRow.u_id].total_replies_watched += +parsedDataRow.total_replies_watched;
        ...
        oThis.deviceIdToReplyDetailsMap[
          parsedDataRow.device_id
        ].total_replies_watched += +parsedDataRow.total_replies_watched;
  ...
  async fetchVideoStatsDetails(batchedData) {
    ...
    for (const userId in userIdToDeviceIdsArray) {
      ...
      let totalVideosWatched = oThis.userIdToVideoDetailsMap[userId]
          ? oThis.userIdToVideoDetailsMap[userId].total_videos_watched
          : 0,
        totalRepliesWatched = oThis.userIdToReplyDetailsMap[userId]
          ? oThis.userIdToReplyDetailsMap[userId].total_replies_watched
          : 0,
      ...
      for (let index = 0; index < deviceIds.length; index++) {
        ...
        if (CommonValidators.validateNonEmptyObject(oThis.deviceIdToVideoDetailsMap[deviceId])) {
          totalVideosWatched += oThis.deviceIdToVideoDetailsMap[deviceId].total_videos_watched;
        ...
        if (CommonValidators.validateNonEmptyObject(oThis.deviceIdToReplyDetailsMap[deviceId])) {
          totalRepliesWatched += oThis.deviceIdToReplyDetailsMap[deviceId].total_replies_watched;
      ...
      batchedData[userId].videos_watched = totalVideosWatched;
      batchedData[userId].replies_watched = totalRepliesWatched;

      if (latestVideoTimestamp.toTimeString() !== new Date(0).toTimeString()) {
        batchedData[userId].last_video_watched_date = latestVideoTimestamp.toDateString();
      }

      if (latestReplyTimestamp.toTimeString() !== new Date(0).toTimeString()) {
        batchedData[userId].last_reply_watched_date = latestReplyTimestamp.toDateString();
      }
    }
  ...
  async uploadData() {
    ...
    for (const userId in oThis.finalData) {
      ...
      const row = [
        ...
        individualUserData.replies_watched,
