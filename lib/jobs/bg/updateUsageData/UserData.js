...
const rootPrefix = '../../../..',
  ...
  stagingUserEmailDataJson = require(rootPrefix + '/test/stagingUserEmailData.json'),
...
class UserData {
  ...
  _checkIfInternalUser(userDbRow) {
    if (basicHelper.isProduction() && (userDbRow.email && stagingUserEmailDataJson[userDbRow.email])) {
      return 1;
    }
  ...
  async fetchMuteUserDetails(batchdata) {
    ...
    for (let index = 0; index < userIds.length; index++) {
      ...
      batchdata[userId].is_muted = cacheResponse.data[userId].all == 1 ? 1 : 0;

      if (batchdata[userId].is_approved) {
