...
class VideoMerge extends ResizeBase {
  ...
  async _fetchEntity() {
    ...
    const videoSegments = await new VideoMergeJobModel().fetchSegmentsByJobId(oThis.jobId);
  ...
  async _updateInMergeJobs() {
    ...
    await new VideoMergeJobModel()
      .update(['status = ?', videoMergeJobConstants.invertedStatuses[videoMergeJobConstants.inProgressStatus]])
