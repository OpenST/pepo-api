...
class ResizeImage extends ResizeBase {
  ...
  async _prepareResizerRequestData() {
    ...
    if (resizeImageKind === imageConstants.profileImageKind) {
      contentType = 'image/jpeg';
    } else {
      extension = util.getFileExtension(oThis.resizeData.source_url);
      contentType = util.getImageContentTypeForExtension(extension);
    }
  ...
  _prepareUrlTemplate() {
    ...
    if (!oThis.image.urlTemplate) {
      let imageUrl = oThis.image.resolutions.original.url,
        fileExtension = null;
      if (oThis.image.kind === imageConstants.profileImageKind) {
        fileExtension = '.jpeg';
      } else {
