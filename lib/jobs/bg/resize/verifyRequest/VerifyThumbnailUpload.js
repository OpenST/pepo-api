...
class VerifyThumbnailUpload {
  ...
  async _checkThumbnailUploaded() {
    ...
    if (resp.isFailure()) {
      ...
      let imageMeta = {};
      imageMeta.size = resp.data.ContentLength;
      imageMeta.width = resp.data.Metadata.width;
      imageMeta.height = resp.data.Metadata.height;
      //Insert in images table and associate the image id in video table.
      await oThis._insertInImagesTable(imageMeta);
   ...
   * @private
   */
  async _insertInImagesTable(imageMeta) {
    const oThis = this;

    let insertParams = imageMeta || {};
    Object.assign(insertParams, {
    ...
    const insertResponse = await imageLib.validateAndSave(insertParams);
