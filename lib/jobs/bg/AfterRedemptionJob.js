...
class AfterRedemptionJob {
  ...
  async _fetchLink() {
    ...
    const userProfileElementsCacheResponse = await userProfileElementsByUserIdCacheObj.fetch();
    if (userProfileElementsCacheResponse.isFailure()) {
      return Promise.reject(userProfileElementsCacheResponse);
    }

    const profileElements = userProfileElementsCacheResponse.data[oThis.currentUserId];

    const profileElementObj = profileElements[userProfileElementConstants.linkIdKind];

    if (CommonValidators.validateNonEmptyObject(profileElementObj)) {
      const linkId = profileElementObj.data;

      const cacheRsp = await new UrlByIdCache({ ids: [linkId] }).fetch();
      if (cacheRsp.isFailure()) {
        return Promise.reject(cacheRsp);
      }

      const link = cacheRsp.data[linkId].url;

      let isHttpBasedUrl = null;

      if (!CommonValidators.isVarNullOrUndefined(link)) {
        isHttpBasedUrl = /^http(s)?:\/\//i.test(link);

        if (isHttpBasedUrl) {
          oThis.profileUrl = link;
        } else {
          oThis.profileUrl = 'http://' + link;
        }
      }
    }
  }
  ...
  _generateMessage() {
    ...
    const message = `*Hi, We have a new Redemption Request*\n
