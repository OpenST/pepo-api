...
class AfterRedemptionJob {
  ...
  async _fetchUser() {
    ...
    oThis.profileImageId = oThis.userData.profileImageId;
  }
  ...
  async _fetchUrl() {
    ...
    if (!CommonValidators.isVarNullOrUndefined(oThis.profileImageId)) {
      const cacheRsp = await new ImageByIdCache({ ids: [oThis.profileImageId] }).fetch();

      if (cacheRsp.isFailure()) {
        return Promise.reject(cacheRsp);
      }

      const urlTemplate = cacheRsp.data[oThis.profileImageId].urlTemplate;
      oThis.profileUrl = shortToLongUrl.getFullUrl(urlTemplate, imageConstants.originalResolution);
    }
  }
