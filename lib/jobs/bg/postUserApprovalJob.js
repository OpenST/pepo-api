...
class PostUserApprovalJob {
  ...
  async _publishAtMentionNotifications(latestVideoId) {
    ...
    //todo-replies: oThis.videoId is not present??? should use mentionedUserIds and video id of last video
  ...
  async _updateTagAndVideoTag(videoId) {
    ...
    oThis.textId = descriptionId;

    const text = await oThis._fetchText(oThis.textId);

    const filterTagsResp = await new FilterTags(text, oThis.textId).perform(),
  ...
  async _filterAtMentions() {
    ...
    //todo-replies: mentionedUserIds should be for last video?
    if (!oThis.textId) {
      return;
    }
