...
class AfterSignUpJob {
  ...
  async _fetchUser(userId) {
    const oThis = this;
    const cacheResp = await new SecureUserCache({ id: userId }).fetch();

    return cacheResp.data;
  }
  ...
  async _addUsersForPepoCampaigns() {
    ...
    // If inviter is present then send email
    // Send email only if its not sent before,
    // if oThis.userInviteCodeId is present means user was part of prelaunch program, so don't send email to inviter
    if (oThis.inviterCodeObj && !oThis.userInviteCodeId) {
      // Send email
  ...
  async _sendEmailToInviter() {
    ...
    if (oThis.inviterCodeObj.userId) {
      let userObj = await oThis._fetchUser(oThis.inviterCodeObj.userId);

      // If user is present and email is opted in.
      if (userObj && userObj.email) {
        let twHandle = await oThis._getTwitterHandle(userObj.id);
        transactionMailParams = {
          receiverEntityId: userObj.id,
          receiverEntityKind: emailServiceApiCallHookConstants.userEmailEntityKind,
    ...
    if (!transactionMailParams) {
      ...
      if (row[0].status == prelaunchInviteConstants.invertedStatuses[prelaunchInviteConstants.doptinStatus]) {
        transactionMailParams = {
          receiverEntityId: row[0].id,
          receiverEntityKind: emailServiceApiCallHookConstants.preLaunchInviteEntityKind,
          customDescription: 'prelaunch signup using invite code',
          templateVars: {
            pepo_api_domain: 1,
            twitter_handle: row[0].handle
          }
        };
      }
    }
    ...
    if (transactionMailParams) {
      transactionMailParams.templateName = emailServiceApiCallHookConstants.userReferredTemplateName;
