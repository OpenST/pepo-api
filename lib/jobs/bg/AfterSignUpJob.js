...
class AfterSignUpJob {
  ...
  async _addUserInviteCode() {
    ...
    let promisesArray = [],
      inviteLimit = oThis.isCreator ? inviteCodeConstants.infiniteInviteLimit : inviteCodeConstants.inviteMaxLimit,
    ...
    if (inviteCodeObj.id) {
      ...
      promisesArray.push(InviteCodeModel.flushCache({ code: insertParams.code, userId: insertParams.user_id }));
    }
