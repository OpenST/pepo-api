...
class AfterSignUpJob {
  ...
  async perform() {
    ...
    const promisesArray = [
      ...
      oThis._muteUserGlobally(),
      oThis._fireApproveUserPixel()
    ];
  ...
  async _addUserInviteCode() {
    ...
    // If user is already present in system but its connecting another account, then do nothing.
    if (oThis.isUserLogin) {
      return;
    }
    const inviteLimit = oThis.isCreator ? inviteCodeConstants.infiniteInviteLimit : inviteCodeConstants.inviteMaxLimit;
    ...
    const insertResponse = await new InviteCodeModel().insert(insertParams).fire();
    ...
    if (oThis.inviterCodeId) {
      const cacheResp = await new InviteCodeByIdCache({ id: oThis.inviterCodeId }).fetch();
  ...
  async _sendEmailToInviter() {
    ...
    const inviterUserData = await oThis._fetchUser(oThis.inviterCodeObj.userId);
    if (inviterUserData && inviterUserData.status !== userConstants.activeStatus) {
      blockedUser = true;
    }
  ...
  /**
   * Fire approve user pixel.
   *
   * @returns {Promise<void>}
   * @private
   */
  async _fireApproveUserPixel() {
    const oThis = this;

    const pixelParameters = {
      approvedViaMedium: pixelConstants.userApprovedViaSignupAutoApprovalMedium, // Mandatory parameter.
      adminId: 0, // Mandatory parameter.
      approvedUserId: oThis.userId // Mandatory parameter.
    };

    await pixelJob.enqueue(pixelJobConstants.approveUserAsCreatorJob, pixelParameters);
  }
}
