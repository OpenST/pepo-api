...
class AfterSignUpJob {
   ...
   * @param {object} params.sanitizedHeaders: request headers.
   *
   * @constructor
   */
  constructor(params) {
    ...
    oThis.pixelParams = {
      ...
      mobile_app_version: oThis.sanitizedHeaders['x-pepo-app-version'],
      e_entity: pixelConstants.userRegisterEntityType, // Mandatory default parameter.
      e_action: pixelConstants.userRegisterEntityType, // Mandatory default parameter.
      p_type: pixelConstants.registerUserPageType // Mandatory default parameter.
    };
  }
  ...
  async perform() {
    ...
    promisesArray = [oThis._addUsersForPepoCampaigns(), oThis._sendPixel()];
    await Promise.all(promisesArray);
  }
  ...
  async _addUserInviteCode() {
    ...
    if (inviteCodeObj.id) {
      ...
      oThis.pixelParams.invite_code = inviteCodeObj.code;
    } else {
      let insertParams = {
    ...
    if (oThis.inviterCodeId) {
      ...
      oThis.pixelParams.invite_code = oThis.inviterCodeObj.code;
  ...
  async _fetchUser(userId) {
    ...
    const userData = cacheResp.data[userId];

    oThis.pixelParams.account_created = userData.createdAt;
  ...
  async _sendEmailToInviter() {
    ...
    if (!transactionMailParams && !blockedUser) {
      ...
      if (row[0] && row[0].status == prelaunchInviteConstants.invertedStatuses[prelaunchInviteConstants.doptinStatus]) {
        ...
        oThis.pixelParams.invite_code = row[0].invite_code;
      }
    }
  ...
  async _insertUtmParams() {
    ...
    if (CommonValidators.validateNonEmptyObject(oThis.utmParams)) {
      ...
      oThis.pixelParams.utm_campaign = oThis.utmParams.utmCampaign;
      oThis.pixelParams.utm_medium = oThis.utmParams.utmMedium;
      oThis.pixelParams.utm_source = oThis.utmParams.utmSource;
    }
  }

  /**
   * Send pixel for user register.
   *
   * @returns {Promise<void>}
   * @private
   */
  async _sendPixel() {
    ...
    await pixelJob.enqueue(pixelJobConstants.userRegisterJob, oThis.pixelParams);
  }
}
