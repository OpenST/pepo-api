...
class Factory {
  ...
  getInstance(messageParams) {
    ...
    switch (messageParams.message.kind) {
      ...
      case bgJobConstants.eventJobTopic: {
        return oThis._eventJobFactory.getInstance(messageParams.message.payload);
      }
      default: {
  ...
  get _eventJobFactory() {
    return require(rootPrefix + '/lib/jobs/bg/eventFactory');
  }
}
