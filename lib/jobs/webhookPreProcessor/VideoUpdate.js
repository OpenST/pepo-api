...
class VideoUpdate {
  ...
  constructor(params) {
    ...
    oThis.videoId = message.payload.videoId;
    ...
    if (hasError) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_j_wpp_vu_vas_1',
  ...
  async _fetchVideoDetail() {
    ...
    oThis.videoDetail = cacheRsp[oThis.videoId];

    if (oThis.videoDetail.status !== videoDetailsConstants.activeStatus) {
      return Promise.reject(
        responseHelper.paramValidationError({
          internal_error_identifier: 'l_j_wpp_vu_fvd_1',
          api_error_identifier: 'invalid_api_params',
