...
class Subscriber {
  ...
  async perform() {
    ...
    await oThis.insertIntoNotificationHook();
  }
  ...
  async sendUnreadNotification() {
    ...
    let notificationUnreadParams = {
      notificationUnread: {
        flag: 1
      }
    };

    let formattedNotificationUnreadParams = new NotificationUnreadFormatter(notificationUnreadParams).perform();
   ...
   * @returns {Promise<void>}
   */
  async insertIntoNotificationHook() {
    const oThis = this;

    // Create entry in notification hooks of mySql.
    await new NotificationHookModel()
      .insert({
        recipients: JSON.stringify([oThis.publishUserId]),
        push_notification_payload: JSON.stringify(oThis.payload),
        execution_timestamp: Date.now() / 1000,
        event_type:
          notificationHookConstants.invertedEventTypes[notificationHookConstants.individualNotificationEventType],
