...
class Subscriber {
  ...
  async perform() {
    ...
    await oThis.incrementUnreadNotificationsCount();
  }
   ...
   * @returns {Promise<void>}
   */
  async incrementUnreadNotificationsCount() {
    const oThis = this;
    if (notificationHookConstants.isPushNotificationEventType(oThis.insertParams.kind)) {
      await new UserNotificationsCountModel().incrementUnreadNotificationCount({ userId: oThis.publishUserId });
    }
  }
}
