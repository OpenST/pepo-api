const rootPrefix = '../../..',
  NotificationJobBase = require(rootPrefix + '/lib/jobs/notification/Base'),
  userNotificationConstants = require(rootPrefix + '/lib/globalConstant/cassandra/userNotification');

/**
 * Class for video transaction receive success.
 *
 * @class VideoTransactionReceiveSuccess
 */
class VideoTransactionReceiveSuccess extends NotificationJobBase {
  /**
   * Constructor for video transaction receive success.
   *
   * @param {object} params
   * @param {string/number} params.actorId
   * @param {string/number} params.subjectUserId
   * @param {string} params.transactionId
   * @param {string/number} params.videoId
   *
   * @constructor
   */
  constructor(params) {
    super();

    const oThis = this;

    oThis.actorId = params.actorId;
    oThis.subjectUserId = params.subjectUserId;
    oThis.transactionId = params.transactionId;
    oThis.videoId = params.videoId;
  }

  /**
   * Return mandatory parameters.
   *
   * @returns {object}
   */
  get mandatoryParams() {
    const oThis = this;

    return {
      'oThis.actorId': oThis.actorId,
      'oThis.subjectUserId': oThis.subjectUserId,
      'oThis.transactionId': oThis.transactionId,
      'oThis.videoId': oThis.videoId
    };
  }

  /**
   * Returns kind of notification.
   *
   * @returns {string}
   */
  get kind() {
    return userNotificationConstants.videoTxReceiveSuccessKind;
  }

  /**
   * Returns class job specific insert parameters.
   *
   * @returns {{}}
   */
  get specificInsertParams() {
    const oThis = this;

    return {
      subject_user_id: oThis.subjectUserId,
      actor_ids: [oThis.actorId],
      actor_count: 1,
      transaction_id: oThis.transactionId,
      video_id: oThis.videoId
    };
  }
}

module.exports = VideoTransactionReceiveSuccess;
