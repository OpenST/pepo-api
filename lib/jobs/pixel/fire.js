...
class FirePixel {
  ...
  async perform(params) {
    ...
    const validationResponse = pixelParamsValidator.validateForPixelIdentifier(pixelIdentifier, pixelVars);
    if (validationResponse.isFailure()) {
      const errorObject = responseHelper.error({
        internal_error_identifier: 'l_j_p_1',
        api_error_identifier: 'something_went_wrong',
        debug_options: { pixelIdentifier: pixelIdentifier, pixelVars: pixelVars }
      });
  ...
  async _getSpecificVars(pixelIdentifier, messagePayload) {
    switch (pixelIdentifier) {
      case pixelConstants.userEntityType: {
  ...
  get _defaultVars() {
    return {
      ...
      mobile_app_version: '' // Intentionally empty. Any pixel which needs to send this will over-write this parameter.
    };
  }
