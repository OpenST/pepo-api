...
class PepoCampaigns {
  ...
  async fetchUsersInfo(emails) {
    ...
    return httpWrapper.makeGetRequest(parsedURL, requestData);
  }
  ...
  async sendTransactionalMail(email, template, emailVars) {
    ...
    return httpWrapper.makePostRequest(resource, requestParams);
  ...
  async addContact(listId, email, attributes, userStatus) {
    ...
    return httpWrapper.makePostRequest(endpoint, requestParams);
  }
}
