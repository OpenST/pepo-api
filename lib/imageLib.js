...
class ImageLib {
  ...
  _shortenExternalUrl(imageUrl) {
    ...
    if (shortUrl.match(imageConstants.twitterImagePrefix)) {
      ...
      let imgPrefix = imageConstants.githubImageUrlPrefix[0].replace('[0-9]', '0');
      shortUrl = imageUrl.replace(imgPrefix, imageConstants.githubImageUrlPrefix[1]);
    } else if (shortUrl.match(imageConstants.googleImagePrefix)) {
      let imgPrefix = imageConstants.googleImageUrlPrefix[0].replace('[0-9]', '3');
      shortUrl = imageUrl.replace(imgPrefix, imageConstants.googleImageUrlPrefix[1]);
    }
