...
class ResizeVideo extends ResizeBase {
  ...
  async _fetchEntity() {
    ...
    await oThis._updateStatus(videoConstants.compressionStartedStatus);
  ...
  async _sendResizerRequest() {
    ...
    if (basicHelper.isEmptyObject(oThis.compressData.compression_data)) {
      await oThis._updateStatus(videoConstants.compressionDoneStatus);
  ...
  async markMediaResizeFailed() {
    ...
    await oThis._updateStatus(videoConstants.compressionFailedStatus);
  }
  ...
  async _updateStatus(status) {
    ...
    await new VideoModel()
      .update({ status: videoConstants.invertedStatuses[status] })
