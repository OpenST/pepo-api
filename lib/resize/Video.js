...
class ResizeVideo extends ResizeBase {
  ...
  async _compressVideos() {
    ...
    const promises = [],
      requestData = oThis.compressData,
      resolutionsHash = {},
      compressionFailed = {};

    for (const compressionSize in oThis.compressData.compression_data) {
      requestData.compression_data[compressionSize] = oThis.compressData[compressionSize];
      ...
      promises.push(compressionPromise);
    }
    ...
    if (status === videoConstants.compressionFailedStatus) {
      const errObject = responseHelper.error({
        internal_error_identifier: 'l_rv_3',
        api_error_identifier: 'something_went_wrong',
