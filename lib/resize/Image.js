...
class ResizeImage extends ResizeBase {
  ...
  async _resizeImages() {
    ...
    if (basicHelper.isEmptyObject(oThis.resizeData.resize_details)) {
      await new ImageModel()
        .update({ status: imageConstants.invertedStatuses[imageConstants.resizeDone] })
        .where({ id: oThis.imageId })
        .fire();
    ...
    if (resizedResponse.isSuccess()) {
      ...
      promisesArray.push(oThis.markMediaResizeFailed());
      promisesArray.push(createErrorLogsEntry.perform(resizedResponse, errorLogsConstants.mediumSeverity));
