...
class ResizeImage {
  ...
  async _resizeImages() {
    ...
    if (resizedResponse.isSuccess()) {
      ...
      let resolutionsHash = {};
      resolutionsHash.original = oThis.currentResolutions.original;
      Object.assign(resolutionsHash, resizedResolutions);
      console.log('-resolutionsHash----', resolutionsHash);
      for (let resolution in resolutionsHash) {
        let longUrl = resolutionsHash[resolution].url,
          params = {};
