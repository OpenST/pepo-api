const rootPrefix = '../..',
  ...
  ImageCache = require(rootPrefix + '/lib/cacheManagement/multi/ImageByIds'),
...
class ResizeImage {
  ...
  async _getImageData() {
    ...
    let imageResponse = await new ImageCache({ ids: [oThis.imageId] }).fetch();
  ...
  async _prepareResizeRequestData() {
    ...
    let resizeImageKind = oThis.image.kind,
      sizesToGenerate = ImageConstants.resizeSizes[resizeImageKind];
  ...
  async _resizeImages() {
    ...
    if (resizedResponse.isSuccess()) {
      ...
      if (!basicHelper.isEmptyObject(resizedResolutions)) {
        ...
        for (let resolution in oThis.currentResolutions) {
          let longUrl = oThis.currentResolutions[resolution].url,
            params = {};
          ...
          let shortenedUrl = imageLib.shortenUrl(params);
          if (shortenedUrl.isFailure()) {
            return Promise.reject(shortenedUrl);
          }
          oThis.currentResolutions[resolution].url = shortenedUrl.data.shortUrl;
        }
        await new ImageModel()
          .update({
      ...
      logger.log('---2------------resizedResponse----------', resizedResponse);
  ...
  async _getSourceUrl() {
    ...
    return oThis.currentResolutions.original.url;
  }
