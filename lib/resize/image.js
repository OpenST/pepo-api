...
class ResizeImage {
  ...
  perform() {
    ...
    return oThis._asyncPerform().catch(async function(err) {
      ...
      await oThis._markImageFailed().catch(function(error) {
        logger.error(`Failed to update image resize status as failed. Error: ${error}`);
      });
  ...
  async _getImageData() {
    ...
    const images = await new ImageModel()
      .select('*')
    ...
    if (!oThis.image || oThis.image.status == imageConstants.invertedStatuses[imageConstants.resizeStarted]) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_r_i_5',
          api_error_identifier: 'something_went_wrong',
  ...
  async _prepareResizeRequestData() {
    ...
    const sourceUrl = await oThis._getSourceUrl(),
    ...
    const resizeImageKind = imageConstants.kinds[oThis.image.kind];
  ...
  async _resizeImages() {
    ...
    if (resizedResponse.isSuccess()) {
      ...
      logger.error('---2------------resizedResponse----------', resizedResponse);
