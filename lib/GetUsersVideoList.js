const rootPrefix = '..',
  ...
  UnapprovedReplyDetailsCache = require(rootPrefix +
  ...
  PendingTransactionsByVideoIdsAndFromUserIdCache = require(rootPrefix +
    '/lib/cacheManagement/multi/PendingTransactionsByVideoIdsAndFromUserId'),
...
class GetUsersVideoList {
  ...
  async _validateParams() {
    ...
    if (oThis.currentUserId) {
      const cacheRsp = await new UserMultiCache({ ids: [oThis.currentUserId] }).fetch();

      if (cacheRsp.isFailure()) {
        return Promise.reject(cacheRsp);
      }

      oThis.currentUser = cacheRsp.data[oThis.currentUserId];
    }
  }
  ...
  async _fetchAllVideoDetails() {
    ...
    if (allVideoIds.length > 0) {
      ...
      oThis._filterChannels();
      oThis._filterOutVideoDetails(oThis.videoDetailsMap, 'videoDetail');
      await oThis._setCurrentUserUnapprovedReplies(allVideoIds);
    }
  }

  /**
   * Filter non active channel ids.
   *
   * @returns {Promise<void>}
   * @private
   */
  async _filterChannels() {
    const oThis = this;
    const allChannelIdsToRemove = [];

    for (const vid in oThis.videoDetailsMap) {
      const vdObj = oThis.videoDetailsMap[vid];
      const channelIds = vdObj.channelIds;
      const channelIdsToRemove = [];
      if (channelIds.length > 0) {
        for (let ind = 0; ind < channelIds.length; ind++) {
          const channelId = channelIds[ind];
          const channel = oThis.channelsMap[channelId];
          if (channel.status !== channelConstants.activeStatus) {
            channelIdsToRemove.push(channelId);
            allChannelIdsToRemove.push(channelId);
          }
        }
        if (channelIdsToRemove.length > 0) {
          vdObj.channelIds = vdObj.channelIds.filter((cid) => !channelIdsToRemove.includes(cid));
        }
      }
      oThis.videoDetailsMap[vid] = vdObj;
    }

    if (allChannelIdsToRemove.length > 0) {
      for (let ind = 0; ind < allChannelIdsToRemove.length; ind++) {
        const cid = allChannelIdsToRemove[ind];
        delete oThis.channelsMap[cid];
      }
    }
  }
  ...
  canForwardEntity(creatorUserId, isReply) {
    ...
    // Todo:global_mute_change is reply parent video creator.
    // Now reply will be seen by everyone or not even by parent depending on the init param-filterUserBlockedReplies
    ...
    if (isReply && !oThis.filterUserBlockedReplies && UserModel.isUserApprovedCreator(userObj)) {
      return true;
    }
  ...
  async _setCurrentUserUnapprovedReplies(vids) {
    const oThis = this;
    ...
    if (!oThis.isAdmin && (!oThis.currentUserId || UserModelKlass.isUserApprovedCreator(oThis.currentUser))) {
      return responseHelper.successWithData({});
    }

    const cacheResp = await new UnapprovedReplyDetailsCache({
      creatorUserId: oThis.currentUserId,
      isAdmin: oThis.isAdmin,
      parentIds: vids
    }).fetch();
    ...
    for (const parentVideoId in cacheResp.data) {
      const totalReplies = cacheResp.data[parentVideoId].totalReplies || 0;
      if (oThis.videoDetailsMap[parentVideoId]) {
        oThis.videoDetailsMap[parentVideoId].totalReplies += totalReplies;
      }
    }
  }
}
