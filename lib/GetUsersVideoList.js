...
class GetUsersVideoList {
  ...
  async _fetchVideoCreators() {
    ...
    // If reply detail ids are present then fetch creator of reply videos
  ...
  async _fetchVideoDetails() {
    ...
    oThis._filterOutVideoDescriptions(oThis.videoDetailsMap);
  }
  ...
  async _fetchReplyDetails() {
    ...
    oThis._filterOutVideoDescriptions(oThis.replyDetailsMap);
  }

  /**
   * Filter out descriptions and links from details map
   *
   * @param detailsMap
   * @private
   */
  _filterOutVideoDescriptions(detailsMap) {
    const oThis = this;
