    ...
    for (const vid in oThis.videoDetailsMap) {
      ...
      if (channelIds.length > 0) {
        for (let ind = 0; ind < channelIds.length; ind++) {
          ...
          if (!channel || channel.status !== channelConstants.activeStatus) {
            channelIdsToRemove[channelId] = true;
            delete oThis.channelsMap[channelId];
          }
        }
        if (CommonValidators.validateNonEmptyObject(channelIdsToRemove)) {
          vdObj.channelIds = vdObj.channelIds.filter((cid) => !channelIdsToRemove[cid]);
        }
      }
      oThis.videoDetailsMap[vid] = vdObj;
    }
