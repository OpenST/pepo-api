const rootPrefix = '..',
  ...
  UnapprovedReplyDetailsCache = require(rootPrefix +
    '/lib/cacheManagement/multi/ReplyDetailsUnapprovedByParentIdAndCreatorUserId.js'),
...
class GetUsersVideoList {
  ...
  async perform() {
    ...
    promisesArray.push(
      ...
      oThis._setCurrentUserUnapprovedReplies()
    );
  ...
  async _setCurrentUserUnapprovedReplies() {
    const oThis = this;

    if (oThis.videoIds.length <= 0 || !oThis.currentUserId || UserModelKlass.isUserApprovedCreator(oThis.currentUser)) {
      return responseHelper.successWithData({});
    }

    const cacheResp = await new UnapprovedReplyDetailsCache({
      creatorUserId: oThis.currentUserId,
      parentIds: oThis.videoIds
    }).fetch();
