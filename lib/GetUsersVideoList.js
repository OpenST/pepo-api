...
class GetUsersVideoList {
  ...
  _filterOutVideoDetails(detailsMap, kind) {
    ...
    for (let key in detailsMap) {
      ...
      if (kind == 'replyDetail') {
        if (vDetail.entityKind == replyDetailConstants.videoEntityKind) oThis.videoIds.push(vDetail.entityId);
        // Append parent video
        if (vDetail.parentKind == replyDetailConstants.videoParentKind) oThis.videoIds.push(vDetail.parentId);
      }
    }
