...
class GetUsersVideoList {
  ...
  async _fetchAllVideoDetails() {
    ...
    oThis.allVideoIds = [...new Set(oThis.allVideoIds)];
  ...
  _filterOutVideoDetails(detailsMap, kind) {
      ...
      if (CommonValidators.validateNonEmptyObject(vDetail)) {
        ...
        if (kind === 'replyDetail') {
          if (vDetail.entityKind === replyDetailConstants.videoEntityKind) {
            ...
            allVideoIds.push(vDetail.entityId);
    ...
    oThis.allVideoIds = oThis.allVideoIds.concat(allVideoIds);
  }
  ...
  async _setCurrentUserVideoRelations() {
    ...
    const cacheResp = await new VideoDistinctReplyCreatorsCache({ videoIds: oThis.allVideoIds }).fetch();
    ...
    for (let ind = 0; ind < oThis.allVideoIds.length; ind++) {
      const vid = oThis.allVideoIds[ind],
