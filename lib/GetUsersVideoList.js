...
class GetUsersVideoList {
  ...
  async _fetchReplyDetailsByVideoIds(videoIds) {
    ...
    console.log('==videoIds============111111111=====', videoIds);
      ...
      if (CommonValidators.validateNonEmptyObject(vDetail)) {
        ...
        if (kind === 'replyDetail') {
          if (vDetail.entityKind === replyDetailConstants.videoEntityKind) {
            ...
            oThis.replyVideosMap[vDetail.id] = vdPayload.videoId;
          }
  ...
  async _fetchVideoContributions() {
    ...
    if (oThis.currentUserId == 1445) {
      console.log(
        oThis.currentUserId,
        '------Primary data from video contributor table:-----',
        videoContributorByVideoIdsAndContributedByUserIdCacheRsp
      );
    }
    ...
    if (oThis.videoIds.length > 0) {
      ...
      if (oThis.currentUserId == 1445) {
        console.log(
          oThis.currentUserId,
          oThis.videoIds,
          'PendingTransactionsByVideoIdsAndFromUserIdCache response--------------------------------------',
          JSON.stringify(cacheRsp)
        );
      }
  ...
  _updateContributionsForPendingTrx() {
    ...
    if (oThis.currentUserId == 1445) {
      console.log(
        '-----------------oThis.videoRspForPendingTransaction----------',
        JSON.stringify(oThis.videoRspForPendingTransaction)
      );
      console.log(
        '\n\n\n\n------11111-----------oThis.replyDetailsMap----------',
        JSON.stringify(oThis.replyDetailsMap)
      );
    }
  ...
  _updateCurrentUserVideoContributions() {
    ...
    if (oThis.currentUserId == 1445) {
      console.log(
        '---------11111--------oThis.currentUserVideoContributionsMap----------',
        JSON.stringify(oThis.currentUserVideoContributionsMap)
      );
    }
    for (const videoId in oThis.videoRspForPendingTransaction) {
    ...
    if (oThis.currentUserId == 1445) {
      console.log(
        '---------2222--------oThis.currentUserVideoContributionsMap----------',
        JSON.stringify(oThis.currentUserVideoContributionsMap)
      );
    }
  }
  ...
  async _setCurrentUserReplyRelations() {
    ...
    const replyVideoIds = Object.values(oThis.replyVideosMap);
    ...
    for (let replyDetailId in oThis.replyVideosMap) {
      const vid = oThis.replyVideosMap[replyDetailId],
      ...
      // oThis.currentUserReplyRelations[vid].isReplyChargeable = 0;
  ...
  async _setCurrentUserVideoRelations() {
    ...
    for (let ind = 0; ind < oThis.videoIds.length; ind++) {
      ...
      if (videoMap.videoDetailId && oThis.videoDetailsMap[videoMap.videoDetailId]) {
        ...
        } else if (replyCreators && replyCreators.length > 0) {
          // If current user belongs to reply creators list then reply is free
          if (replyCreators.includes(Number(oThis.currentUserId))) {
            oThis.currentUserVideoRelations[vid].isReplyChargeable = 0;
          }
        }
      }
