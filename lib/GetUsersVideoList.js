...
class GetUsersVideoList {
   ...
   * @param {number} [params.profileUserId] || sent if admin views a particular profile
  ...
  async _setCurrentUserUnapprovedReplies(vids) {
    ...
    if (oThis.isAdmin && oThis.profileUserId) {
      creatorUserId = oThis.profileUserId;
    } else if (oThis.currentUserId && !UserModelKlass.isUserApprovedCreator(oThis.currentUser)) {
    ...
    for (let parentVideoId in cacheResp.data) {
