...
class GetUsersVideoList {
  ...
  async perform() {
    ...
    promisesArray.push(
      ...
      oThis._fetchCurrentUserViewHistory()
    );
  ...
  async _fetchReplyDetailsByVideoIds(videoIds) {
    ...
    const ReplyDetailsByVideoIdsCacheObj = new ReplyDetailsByVideoIds({
        entityIds: videoIds,
  ...
  _updateContributionsForPendingTrx() {
    ...
    for (let videoId in oThis.videoRspForPendingTransaction) {
      ...
      const isReplyEntity = videoEntity.replyDetailId ? true : false;
  ...
  /**
   * Fetch current user video viewing history
   *
   * @returns {Promise<void>}
   * @private
   */
  async _fetchCurrentUserViewHistory() {
    const oThis = this;
