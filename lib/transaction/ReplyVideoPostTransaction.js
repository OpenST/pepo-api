...
class ReplyVideoPostTransaction {
  ...
  async perform() {
    ...
    const replySenderNotificationParams = {
    ...
    const replyReceiverNotificationParams = {
      userId: oThis.videoDetails.creatorUserId,
      replyDetailId: oThis.replyDetail.id,
      videoId: oThis.replyDetail.entityId
    };
    ...
    if (oThis.pepoAmountInWei === '0') {
      ...
      await notificationJobEnqueue.enqueue(
        notificationJobConstants.replySenderWithoutAmount,
        replySenderNotificationParams
      );

      // Notification would be published only if user is approved.
      await notificationJobEnqueue.enqueue(
        notificationJobConstants.replyReceiverWithoutAmount,
        replyReceiverNotificationParams
      );
    } else {
      // Notification would be published only if user is approved.
      replySenderNotificationParams.amount = oThis.pepoAmountInWei;
      await notificationJobEnqueue.enqueue(
        notificationJobConstants.replySenderWithAmount,
        replySenderNotificationParams
      );

      // Notification would be published only if user is approved.
      replyReceiverNotificationParams.amount = oThis.pepoAmountInWei;
      await notificationJobEnqueue.enqueue(
        notificationJobConstants.replyReceiverWithAmount,
        replyReceiverNotificationParams
      );
    }
