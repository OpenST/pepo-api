const rootPrefix = '../..',
  ...
  ChannelUserByUserIdsCache = require(rootPrefix + '/lib/cacheManagement/multi/channel/ChannelUserByUserIds'),
...
class SortUnseenFeed {
  ...
  async perform() {
    ...
    if (oThis.currentUserId) {
      ...
      promise2.push(oThis._getUserChannelList());
    }
  ...
  async _sortFeeds() {
    ...
    const sortParams = {
      ...
      joinedChannelIdMap: oThis.joinedChannelIdMap,
  ...
  /**
   * Get channel list which current user has joined.
   *
   * Sets oThis.joinedChannelIdMap
   *
   * @returns {Promise<void>}
   * @private
   */
  async _getUserChannelList() {
    const oThis = this;

    const channelUsersCacheRsp = await new ChannelUserByUserIdsCache({ userIds: [oThis.currentUserId] }).fetch();

    if (channelUsersCacheRsp.isFailure()) {
      return Promise.reject(channelUsersCacheRsp);
    }

    for (let i = 0; i < channelUsersCacheRsp.data[oThis.currentUserId].length; i++) {
      const cid = channelUsersCacheRsp.data[oThis.currentUserId][i];
      oThis.joinedChannelIdMap[cid] = 1;
    }
  }
  ...
  async _getCurrentUserTwitterUserConnections() {
    ...
    for (let index = 0; index < oThis.allUserIds.length; index++) {
      ...
      // Every user may not be twitter user.
      if (twitterUserCacheRespData[userId].id) {
    ...
    if (twitterUser1Id && twitterUser2Ids.length > 0) {
