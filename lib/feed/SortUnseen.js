...
//todo: new cache clear
//todo: use user_contributors
/**
...
class SortUnseenFeed {
  ...
  async _fetchActorIds() {
    ...
    for (let index = 0; index < oThis.unseenFeedIds.length; index++) {
      ...
      if (!basicHelper.isEmptyObject(oThis.feedsMap[unseenFeedId])) {
        ...
        let actor = oThis.feedsMap[unseenFeedId].actor;

        oThis.actorIdToFeedIdsMap[actor] = oThis.actorIdToFeedIdsMap[actor] || [];
        oThis.actorIdToFeedIdsMap[actor].push(unseenFeedId);
      }
    }
  ...
  async _getCurrentUserTwitterUserConnections() {
    ...
    //todo: duplicate currentUserId posible
    ...
    for (let index = 0; index < twitterUser2Ids.length; index++) {
      ...
      if (!basicHelper.isEmptyObject(twitterUserConnectionObj)) {
        if (
          ...
          relevantFeedIds = relevantFeedIds.concat(feedIdsForUserId);
        } else {
          irrelevantFeedIds = irrelevantFeedIds.concat(feedIdsForUserId);
        }
      }
    }
