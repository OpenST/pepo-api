...
class Sort {
  ...
  async _performSort() {
    ...
    for (let i = 0; i < oThis.allFeedIds.length; i++) {
      ...
      const isUnseenVideo = oThis._isUnseenVideo(videoId),
        ...
        isUnseenAfterMyLastReply = oThis._isUnseenAfterMyLastReply(feedId);
      ...
      isUnseenAfterMyLastReply: ${isUnseenAfterMyLastReply}
      ...
      } else if (iHaveReplied && isUnseenAfterMyLastReply) {
  ...
  _isUnseenAfterMyLastReply(feedId) {
    const oThis = this;
    ...
    return (
      myLastReplyTimestampOnFeed > 0 &&
      myLastReplyTimestampOnFeed >= oThis._videoLastSeenTimestamp(videoId)
    );
  }
