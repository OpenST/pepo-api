...
class Sort {
  ...
  async _performForLoggedOutFeed() {
    ...
    const uniqueUnpopularActorVideosData = {};

    const sortData = {
      popular: []
    };

    for (let i = oThis.allFeedIds.length - 1; i >= 0; i--) {
      ...
      actorId:${actorId},
      unpopular_category_unique_actor:${uniqueUnpopularActorVideosData[actorId] ? 0 : 1}
      `);
      }
      ...
      if (isPopularFeed) {
        ...
        // UnPopular video.
        uniqueUnpopularActorVideosData[actorId] = uniqueUnpopularActorVideosData[actorId] || [];
        uniqueUnpopularActorVideosData[actorId].push(feedId);
      }
    }

    if (oThis._isLoggingEnabled()) {
      logger.info(
        `\n================uniqueUnpopularActorVideosData:${JSON.stringify(
          uniqueUnpopularActorVideosData
        )} ------------\n`
      );
    }

    const unpopularFeedIds = oThis._getFeedIdsForUnpopularCategory(uniqueUnpopularActorVideosData);

    // Note: Reverse the order as the order of above loop was based on oldest feed first
    oThis._sortFeedsOnPopularityScore(sortData.popular);
   ...
   * @returns {Promise<void>}
   */
  _getFeedIdsForUnpopularCategory(uniqueUnpopularActorVideosData) {
    const oThis = this;

    const uniqueFeedIds = [];

    for (let actorId in uniqueUnpopularActorVideosData) {
      const feedIds = uniqueUnpopularActorVideosData[actorId];
      oThis._sortFeedsOnPopularityScore(feedIds);
      uniqueFeedIds.push(feedIds[0]);
    }
    //sort on time
    return oThis._sortFeedsOnPublishTime(uniqueFeedIds);
  }
  ...
  async _performSort() {
    ...
    // If video belongs to category 5 and category 8, and video is skipped for category 5 due to uniqueness, then still goes into category 8
    ...
    for (let i = oThis.allFeedIds.length - 1; i >= 0; i--) {
      ...
      category5_unique_actor:${uniqueActorVideosData['category5'][actorId] ? 0 : 1},
      category9_unique_actor:${uniqueActorVideosData['category9'][actorId] ? 0 : 1},
      category11_unique_actor:${uniqueActorVideosData['category11'][actorId] ? 0 : 1},
      unpopular_unique_actor:${uniqueActorVideosData['unpopular'][actorId] ? 0 : 1}
      `);
      }
      ...
      if (isUnseenVideo && isMyVideo) {
      ...
      } else if (isUnseenVideo && hasContributedToAuthor && !uniqueActorVideosData['category5'][actorId]) {
        // Unseen video from author's I have contributed to. ----------------------------
        sortData.category5.push(feedId);
        uniqueActorVideosData['category5'][actorId] = feedId;
        ...
        // UnPopular video.
        uniqueActorVideosData['unpopular'][actorId] = uniqueActorVideosData['unpopular'][actorId] || [];
        uniqueActorVideosData['unpopular'][actorId].push(feedId);
      }
    }

    if (oThis._isLoggingEnabled()) {
      logger.info(
        `\n================uniqueActorVideosData['unpopular']:${JSON.stringify(
          uniqueActorVideosData['unpopular']
        )} ------------\n`
      );
    }

    const unpopularFeedIds = oThis._getFeedIdsForUnpopularCategory(uniqueActorVideosData['unpopular']);
   ...
   * @returns {Promise<void>}
   */
  _sortFeedsOnPopularityScore(feedIds) {
    const oThis = this;
