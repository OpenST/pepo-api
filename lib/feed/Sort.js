...
class Sort {
  ...
  async _performForLoggedOutFeed() {
    ...
    for (let i = oThis.allFeedIds.length - 1; i >= 0; i--) {
      ...
      } else if (!uniqueUnpopularActorVideosData[actorId]) {
        // UnPopular video.
        sortData.unpopular.push(feedId);
        uniqueUnpopularActorVideosData[actorId] = feedId;
      }
    }

    // Note: Reverse the order as the order of above loop was based on oldest feed first
    sortData.unpopular.reverse();
    ...
    const sortedFeedIds = sortData.popular.concat(sortData.unpopular);

    const oldestFeedObj = oThis.allFeedMap[oThis.allFeedIds.length - 1] || {},
  ...
  async _performSort() {
    ...
    for (let i = oThis.allFeedIds.length - 1; i >= 0; i--) {
      ...
      if (isUnseenVideo && isMyVideo) {
        ...
        // Popular video.
        sortData.popular.push(feedId);
      } else if (!uniqueActorVideosData['unpopular'][actorId]) {
        // UnPopular video.
        sortData.unpopular.push(feedId);
        uniqueActorVideosData['unpopular'][actorId] = feedId;
      }
    }
