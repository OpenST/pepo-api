...
class Sort {
   ...
   * @param {Boolean} params.sortFeeds
  ...
  async _performSort() {
    ...
    for (let i = 0; i < oThis.allFeedIds.length; i++) {
      ...
      if (isUnseenVideo && isMyVideo) {
        ...
        // It should already be seen as the above condition in if is present
        const wasPublishedRecently =
          (feedObj.paginationIdentifier >= Date.now() / 1000) - feedConstants.personalizedFeedSeenVideosAgeInSeconds;
  ...
  _videoLastSeenTimestamp(videoId) {
    const oThis = this;
    const userVideoViewObj = oThis.videoViewMap[videoId] || { lastViewAt: 0 };

    return Math.max(userVideoViewObj.lastViewAt || 0, userVideoViewObj.lastReplyViewAt || 0);
  }
  ...
  _lastReplyTimestampByCurrentUserOnVideo(videoId) {
    ...
    return videoRelatedActionsObj.lastReplyTimestamp || 0;
  }
