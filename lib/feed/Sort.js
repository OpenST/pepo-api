...
class Sort {
  ...
  async _performForLoggedOutFeed() {
    ...
    const unpopularFeedIds = oThis._getFeedIdsWithUniquenessCategory(sortData.unpopular);
  ...
  async _performSort() {
    ...
    for (let i = 0; i < oThis.allFeedIds.length; i++) {
      ...
      category7_unique_actor:${uniqueActorVideosData['category7'][actorId]},
      category11_unique_actor:${uniqueActorVideosData['category11'][actorId]},
      category15_unique_actor:${uniqueActorVideosData['category15'][actorId]},
      category16_unique_actor:${uniqueActorVideosData['category16'][actorId]},
      category18_unique_actor:${uniqueActorVideosData['category18'][actorId]},
      category21_unique_actor:${uniqueActorVideosData['category21'][actorId]}
      `);
      }
      ...
      if (isUnseenVideo && isMyVideo) {
        ...
        !uniqueActorVideosData['category7'][actorId] &&
      ...
      } else if (isUnseenVideo && hasContributedToAuthor && !uniqueActorVideosData['category11'][actorId]) {
        ...
        isJoinedChannelVideo &&
        !uniqueActorVideosData['category15'][actorId]
      ) {
        ...
        isUnseenVideo &&
        oThis._isFollowingOnTwitter(actorId) &&
        !uniqueActorVideosData['category16'][actorId]
      ) {
      ...
      } else if (isUnseenVideo && isJoinedChannelVideo && !uniqueActorVideosData['category18'][actorId]) {
      ...
      } else if (isUnseenVideo && !uniqueActorVideosData['category21'][actorId]) {
        // Unseen videos.
    ...
    if (oThis._isLoggingEnabled()) {
      logger.info(
        `\n================uniqueActorVideosData['category24']:${JSON.stringify(
          uniqueActorVideosData['category24']
        )} ------------\n================uniqueActorVideosData['category26']:${JSON.stringify(
          uniqueActorVideosData['category26']
        )} ------------\n`
      );
    }
    ...
    for (let categoryType in sortData) {
      ...
      if (popularitySortCategories[categoryType]) {
        ...
        sortData[categoryType] = oThis._getFeedIdsWithUniquenessCategory(feedIds);
      }
    }
   ...
   * @returns {Promise<void>}
   */
  _getFeedIdsWithUniquenessCategory(sortedFeedIds) {
    const oThis = this;
