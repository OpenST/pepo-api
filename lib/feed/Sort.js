...
class Sort {
  ...
  async perform() {
    ...
    const deviceOs = headerHelper.pepoDeviceOs(oThis.headers),
      buildNumber = headerHelper.pepoBuildNumber(oThis.headers);
    ...
    logger.info('Is reply build ====== ', headerHelper.isReplyBuild(params));
    logger.info('Is video play event build ====== ', headerHelper.isVideoPlayEventBuild(params));

    if (headerHelper.isReplyBuild(params)) {
      oThis.isReplyBuild = true;
      sortFeed = true;
    } else if (headerHelper.isVideoPlayEventBuild(params)) {
      sortFeed = true;
    }
  ...
  async _performSort() {
    ...
    for (let i = 0; i < oThis.allFeedIds.length; i++) {
      ...
      hasContributedToUser:${oThis._hasContributedToUser(actorId)}
      ...
      if (isUnseenVideo && isMyVideo) {
        ...
        // My Video And new reply (Republish on top)
        sortData.category2.push(feedId);
      } else if (oThis._lastReplyTimestampByCurrentUserOnVideo(videoId) > 0 && hasNewReplyByOtherUsers) {
        // I had replied to a video and has a new reply(Republish on top)
        sortData.category3.push(feedId);
      } else if (isUnseenVideo && oThis._hasContributedToUser(actorId)) {
      ...
      } else if (oThis._hasContributedToUser(actorId) && hasNewReplyByOtherUsers) {
  ...
  _isFollowingOnTwitter(userId) {
    ...
    const twitterConnectionObj = oThis.twitterConnectionsMap[userId];

    return CommonValidators.validateNonEmptyObject(twitterConnectionObj);
  }
