 ...
 *
 *
 var mysqlQuery = require('./lib/queryBuilders/mysql');
 var mysqlWrapper = require('./lib/mysqlWrapper');
...
class MySqlQueryBuilder {
  ...
  insertMultiple(insertColumns, insertValues, insertOptions) {
    ...
    // insert values
    let totalColumnsToInsert = oThis.insertIntoColumns.length;
  ...
  where(whereConditions) {
    ...
    if (typeof whereConditions === 'string') {
      ...
      // For sub-queries create string locally and push it to sub-queries array by joining with AND.
      // Also push key and value alternatively in local replacement array.
      let whereColumns = Object.keys(whereConditions),
  ...
  order_by(orderByConditions) {
    ...
    if (Array.isArray(orderByConditions)) {
      ...
      // Also push key and value alternatively in local replacement array.
      let orderColumns = Object.keys(orderByConditions),
   ...
   * @return {object<self>} oThis
   */
  onDuplicate(onDuplicateConditions) {
    const oThis = this;
    ...
    if (typeof onDuplicateConditions === 'string') {
      ...
      // For sub-queries create string locally and push it to sub-queries array by joining with COMMA.
      // Also push key and value alternatively in local replacement array.
      let onDuplicateColumns = Object.keys(onDuplicateConditions),
        onDuplicateValues = Object.values(onDuplicateConditions),
        localSubQueries = [],
        localReplacements = [];

      if (onDuplicateColumns.length > 0) {
        for (let i = 0; i < onDuplicateColumns.length; i++) {
          localSubQueries.push('??=?');
  ...
  _generateUpdate() {
    ...
    if (oThis.touchUpdatedAt) {
      ...
      oThis.updateSubQueriesReplacement = oThis.updateSubQueriesReplacement.concat([Math.round(new Date() / 1000)]);
    }
