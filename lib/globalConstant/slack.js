...
class Slack {
  ...
  addApproveBlockSection(userId, isDoubleOptInDone) {
    ...
    let elements = [oThis.approveUserElememt(userId), oThis.deleteUserElememt(userId)];

    if (isDoubleOptInDone) {
      elements.push(oThis.sendEmailForResubmissionElememt(userId));
    }
  ...
  addUnmuteBlockSection(userId, isDoubleOptInDone) {
    ...
    let elements = [oThis.unmuteUserElememt(userId), oThis.deleteUserElememt(userId)];
    ...
    return unmuteSection;
  }

  unmuteUserElememt(userId) {
    const oThis = this;

    let unmuteUserValue = `${oThis.unmuteUserEventType}|user_id:${userId}`;
  ...
  deleteUserElememt(userId) {
    const oThis = this;

    let deleteUserValue = `${oThis.deleteUserEventType}|user_id:${userId}`;
  ...
  sendEmailForResubmissionElememt(userId) {
    const oThis = this;

    let sendEmailValue = `${oThis.sendEmailEventType}|user_id:${userId}`;
