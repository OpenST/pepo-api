...
class Slack {
   ...
   * @returns {{elements: *, type: string}}
   */
  addChannelBlockSection(channelId) {
    const oThis = this;

    const elements = [oThis.deleteChannelElement(channelId)];
  ...
  get allowedEventTypes() {
    ...
    return [
      ...
      oThis.deleteChannelEventType
    ];
  }
  // Slack event types end.

  // Slack channel monitoring constants start
  ...
  // Slack channel monitoring constants end
}
