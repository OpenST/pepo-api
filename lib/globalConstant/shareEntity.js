...
class ShareEntity {
  ...
  getChannelShareEntity(params) {
    ...
    if (params.channelTagline) {
      const truncateLimit = textConstants.truncatedChannelTaglineLimit;
      const truncatedTagline =
        params.channelTagline.length > truncateLimit
          ? params.channelTagline.toString().substring(0, truncateLimit) + '...'
          : params.channelTagline;
      message = `🌶️ Join ${params.channelName} on Pepo! ${pepoTwitterHandle} - ${truncatedTagline} - ${params.url}`;
    } else {
      message = `🌶️ Join ${params.channelName} on Pepo! ${pepoTwitterHandle} - ${params.url}`;
    }
    return {
      message: message.replace(/&amp;/g, '&'),
      title: `🌶️ Join ${params.channelName} on Pepo!`, // Optional.
      subject: 'Meet the people shaping the crypto movement.', // Optional.
      url: params.url
    };
  }
