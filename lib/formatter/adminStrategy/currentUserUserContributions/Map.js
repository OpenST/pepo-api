const rootPrefix = '../../../..',
  ...
  currentUserUserContributionSingleFormatter = require(rootPrefix +
    '/lib/formatter/adminStrategy/currentUserUserContributions/Single'),
  responseHelper = require(rootPrefix + '/lib/formatter/response');

class currentUserUserContributionMapFormatter extends BaseFormatter {
  /**
   * Constructor for current user user contributions map formatter.
   *
   * @param {object} params
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.currentUserUserContributionsMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_as_cuuc_1',
