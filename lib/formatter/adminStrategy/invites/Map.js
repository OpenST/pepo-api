const rootPrefix = '../../../..',
  ...
  InviteUserSingleFormatter = require(rootPrefix + '/lib/formatter/adminStrategy/invites/Single'),
...
class InviteUserMapFormatter extends BaseFormatter {
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.inviteMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_as_i_m_1',
  ...
  _format() {
    ...
    for (const id in oThis.inviteMap) {
      const formattedInviteUser = new InviteUserSingleFormatter({ inviteUser: oThis.inviteMap[id] }).perform();
