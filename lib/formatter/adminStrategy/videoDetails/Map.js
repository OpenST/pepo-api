const rootPrefix = '../../../..',
  ...
  VideoDetailSingleFormatter = require(rootPrefix + '/lib/formatter/adminStrategy/videoDetails/Single'),
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.videoDetailsMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_as_vd_m_1',
  ...
  _format() {
    ...
    for (const videoDetailId in oThis.videoDetailsMap) {
      const videoDetailObj = oThis.videoDetailsMap[videoDetailId],
        formattedVideoDetailRsp = new VideoDetailSingleFormatter({ videoDetail: videoDetailObj }).perform();
