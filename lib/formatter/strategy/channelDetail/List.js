  ...
  _validate() {
    ...
    if (!CommonValidators.validateArray(oThis.channelIds)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_cd_l_1',
    ...
    if (!CommonValidators.validateObject(oThis.channelDetailsMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_cd_l_2',
        api_error_identifier: 'entity_formatting_failed',
  ...
  _format() {
    ...
    for (let index = 0; index < oThis.channelIds.length; index++) {
      const channelId = oThis.channelIds[index],
        channel = oThis.channelDetailsMap[channelId];

      const formattedChannel = new ChannelDetailSingleFormatter({ channel: channel }).perform();
