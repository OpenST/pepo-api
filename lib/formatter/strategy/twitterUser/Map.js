const rootPrefix = '../../../..',
  ...
  twitterUserSingleFormatter = require(rootPrefix + '/lib/formatter/strategy/twitterUser/Single'),
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.twitterUsersMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_tu_m_1',
  ...
  _format() {
    ...
    for (const userId in oThis.twitterUsersMap) {
      const twitterUserObj = oThis.twitterUsersMap[userId],
        formattedTwitterUsersRsp = new twitterUserSingleFormatter({ twitterUser: twitterUserObj }).perform();
