const rootPrefix = '../../../..',
  ...
  AdminUserSingleFormatter = require(rootPrefix + '/lib/formatter/strategy/adminUser/Single'),
...
class UserMapFormatter extends BaseFormatter {
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.usersByIdMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_au_m_1',
    ...
    if (!CommonValidators.validateObject(oThis.tokenUsersByUserIdMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_au_m_2',
        api_error_identifier: 'entity_formatting_failed',
        debug_options: {
  ...
  _format() {
    ...
    for (const userId in oThis.usersByIdMap) {
      ...
      const formattedUser = new AdminUserSingleFormatter({ user: userObj, tokenUser: tokenUser }).perform();
