  ...
  _format() {
    ...
    for (let videoId in oThis.unseenRepliesByVideoIdMap) {
      ...
      for (let index = 0; index < unseenRepliesArray.length; index++) {
        const singleUnseenReply = unseenRepliesArray[index];

        finalResponse[videoId].unseen.push({
          reply_detail_id: Number(
            singleUnseenReply[replyDetailConstants.longToShortNamesMapForCache['replyDetailsId']]
          ),
          user_id: Number(singleUnseenReply[replyDetailConstants.longToShortNamesMapForCache['creatorUserId']])
        });
      }
    }
