...
class TagSingleFormatter extends BaseFormatter {
  ...
  _validate() {
    ...
    const tagKeyConfig = {
      ...
      weight: { isNullAllowed: false },
      status: { isNullAllowed: false },
      createdAt: { isNullAllowed: false },
      updatedAt: { isNullAllowed: false }
    };

    return oThis._validateParameters(oThis.tag, tagKeyConfig);
  }
  ...
  _format() {
    ...
    return responseHelper.successWithData({
      id: oThis.tag.id,
      text: oThis.tag.name,
      weight: oThis.tag.weight,
      status: oThis.tag.status,
      created_at: oThis.tag.createdAt,
      updated_at: oThis.tag.updatedAt
    });
  }
}
