...
class SearchCategoryListFormatter extends BaseFormatter {
  ...
  constructor(params) {
    ...
    oThis[entityType.searchCategoriesList] = params[entityType.searchCategoriesList];
  }
  ...
  _validate() {
    ...
    if (!CommonValidators.validateArray(oThis[entityType.searchCategoriesList])) {
  ...
  _format() {
    ...
    for (let index = 0; index < oThis[entityType.searchCategoriesList].length; index++) {
      const formattedFeedRsp = new SearchCategorySingleFormatter({
        searchCategoryResult: oThis[entityType.searchCategoriesList][index]
      }).perform();
