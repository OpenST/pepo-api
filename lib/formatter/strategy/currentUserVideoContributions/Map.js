const rootPrefix = '../../../..',
  ...
  currentUserVideoContributionSingleFormatter = require(rootPrefix +
    '/lib/formatter/strategy/currentUserVideoContributions/Single'),
...
class currentUserVideoContributionMapFormatter extends BaseFormatter {
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.currentUserVideoContributionsMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_cuvc_1',
  ...
  _format() {
    ...
    for (const videoId in oThis.currentUserVideoContributionsMap) {
      const currentUserVideoContribution = oThis.currentUserVideoContributionsMap[videoId],
      ...
      finalResponse[videoId] = formattedCurrentUserVideoContributionRsp.data.totalAmount;
    }
