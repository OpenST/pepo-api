...
class UserSearchListFormatter extends BaseFormatter {
  ...
  constructor(params) {
    ...
    oThis[entityType.userSearchList] = params[entityType.userSearchList];
  }
  ...
  _validate() {
    ...
    if (!CommonValidators.validateArray(oThis[entityType.userSearchList])) {
  ...
  _format() {
    ...
    for (let index = 0; index < oThis[entityType.userSearchList].length; index++) {
      const formattedFeedRsp = new UserSearchSingleFormatter({
        userSearchResult: oThis[entityType.userSearchList][index]
      }).perform();
