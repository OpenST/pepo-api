...
/**
 * Class for logged in user map formatter.
 *
 * @class LoggedInUserMapFormatter
 */
class LoggedInUserMapFormatter extends BaseFormatter {
  /**
   * Constructor for logged in user map formatter.
   *
   * @param {object} params
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.usersByIdMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_liu_m_1',
    ...
    if (!CommonValidators.validateObject(oThis.tokenUsersByUserIdMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_liu_m_2',
        api_error_identifier: 'entity_formatting_failed',
        debug_options: {
  ...
  _format() {
    ...
    for (const userId in oThis.usersByIdMap) {
      ...
      const formattedUser = new LoggedInUserSingleFormatter({ user: userObj, tokenUser: tokenUser }).perform();
...
module.exports = LoggedInUserMapFormatter;
