const rootPrefix = '../../../..',
  ...
  VideoSingleFormatter = require(rootPrefix + '/lib/formatter/strategy/video/Single'),
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.videoMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_v_m_1',
  ...
  _format() {
    ...
    for (const videoId in oThis.videoMap) {
      const videoObj = oThis.videoMap[videoId],
        formattedVideoRsp = new VideoSingleFormatter({ video: videoObj }).perform();
