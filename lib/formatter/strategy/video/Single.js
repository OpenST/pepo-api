...
class VideoSingleFormatter extends BaseFormatter {
  ...
  _validate() {
    ...
    if (oThis.video.posterImageId) {
      oThis._validateParameters(oThis.video, videoKeyConfig);
  ...
  _format() {
    ...
    for (let imageSize in oThis.video.resolutions) {
      let imageSizeResolution = oThis.video.resolutions[imageSize];
      let resolutionUrl = imageSizeResolution.url;
      imageSizeResolution['url'] = resolutionUrl.replace(
        s3Constants.videoShortUrlPrefix,
        s3Constants.shortToLongUrl[s3Constants.videoShortUrlPrefix]
      );
      oThis.video.resolutions[imageSize] = imageSizeResolution;
    }
