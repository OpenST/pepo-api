...
class PaymentReceiptFormatter extends BaseFormatter {
  ...
  _validate() {
    ...
    const receiptKeyConfig = {
      id: { isNullAllowed: false },
      rawReceipt: { isNullAllowed: false },
      status: { isNullAllowed: false },
      updatedAt: { isNullAllowed: false }
    };

    return oThis.validateParameters(oThis.paymentReceipt, receiptKeyConfig);
  }
  ...
  _format() {
    ...
    return responseHelper.successWithData({
      id: Number(oThis.paymentReceipt.id),
      rawReceipt: oThis.paymentReceipt.rawReceipt,
      status: oThis.paymentReceipt.status,
      uts: Number(oThis.paymentReceipt.updatedAt)
    });
  }
}
