const rootPrefix = '../../../..',
  ...
  PricePointsSingleFormatter = require(rootPrefix + '/lib/formatter/strategy/pricePoints/Single'),
  ...
  _validate() {
    ...
    if (!CommonValidators.validateObject(oThis.pricePointsMap)) {
      return responseHelper.error({
        internal_error_identifier: 'l_f_s_pp_m_1',
  ...
  _format() {
    ...
    for (const stakeCurrency in oThis.pricePointsMap) {
      if (stakeCurrency !== 'OST') {
        return responseHelper.error({
          internal_error_identifier: 'l_f_s_pp_m_2',
          api_error_identifier: 'entity_formatting_failed',
