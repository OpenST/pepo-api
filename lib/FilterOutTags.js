...
class FilterOutTags {
  ...
  async _add_update_tags() {
    ...
    for (let i = 0; i < dbResp.length; i++) {
      const row = dbResp[i];
      if (oThis.tagNames.hasOwnProperty(row.name)) {
        oThis.tagIds.push(row.id);
        oThis.tagNames[row.name] = 0;
      }
    }
    ...
    if (CommonValidator.validateObject(oThis.tagNames)) {
      ...
      for (let tagName in oThis.tagNames) {
        if (oThis.tagNames[tagName]) {
          insertArray.push([tagName, 1, tagConstants.invertedStatuses[tagConstants.activeStatus]]);
