...
class PaymentProcessBase {
   ...
   * @private
   */
  async _handleFailure(internalErrorIdentifier, status, errorData) {
    const oThis = this;
  ...
  async _markPaymentStatus(status, errorData) {
    ...
    if (status) {
      if (status === fiatPaymentConstants.receiptValidationPendingStatus) {
  ...
  _getRetryAfterTime(retryCount) {
    ...
    return basicHelper.getCurrentTimestampInSeconds() + 10 * retryCount + 10;
  }
  /**
