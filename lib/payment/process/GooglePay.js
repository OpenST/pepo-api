...
class ProcessGooglePay extends PaymentProcessBase {
  ...
  async _validateReceipt() {
    ...
    let receiptData =
      typeof oThis.paymentReceipt.transactionReceipt == 'string'
        ? JSON.parse(oThis.paymentReceipt.transactionReceipt)
        : oThis.paymentReceipt.transactionReceipt;

    const url = Util.format(requestUrl, receiptData.packageName, receiptData.productId, receiptData.purchaseToken);
