...
const REQUEST_URL = 'https://www.googleapis.com/androidpublisher/v3/applications/%s/purchases/products/%s/tokens/%s';
  ...
  async _validateReceipt() {
    ...
    return authClient
        ...
        if (resp.status && resp.status === 200) {
          ...
          if (coreConstants.environment === 'production') {
            ...
            if (resp.data.purchaseState === 0) {
              ...
              oThis.status = fiatPaymentConstants.receiptValidationCancelledStatus;
              return oThis._handleFailure('l_p_p_gp_9', oThis.status, {});
            } else {
              oThis.status = fiatPaymentConstants.receiptValidationPendingStatus;
      ...
      .catch(async function(error) {
        oThis.receiptResponseData = error;
...
// rootPrefix = '.'
// coreConstants = require(rootPrefix + '/config/coreConstants')
// REQUEST_URL = 'https://www.googleapis.com/androidpublisher/v3/applications/com.pepo.staging/purchases/voidedpurchases';
//
// authClient = new GoogleAuthLibrary.JWT(
//   coreConstants.GOOGLE_INAPP_SERVICE_ACCOUNT_EMAIL,
//   null,
...
// )
//
// url = Util.format(REQUEST_URL);
//
// authClient.request({ url }).then(async function(resp) {console.log('-------------------------resp----', JSON.stringify(resp))})
//
