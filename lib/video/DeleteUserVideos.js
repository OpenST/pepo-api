...
class DeleteUserVideos {
  ...
  async _changeS3Permissions(videoIds) {
    ...
    await oThis._changePermissionsForAllResolutions(cacheRsp.data); // For videos
    ...
    await oThis._changePermissionsForAllResolutions(imageCacheRsp.data); // For poster images
  }
   ...
   * @private
   */
  async _changePermissionsForAllResolutions(entityHash) {
    const oThis = this;
    ...
    for (const entityId in entityHash) {
      const entity = entityHash[entityId],
        ...
        urlTemplate = entity.urlTemplate.match(/\/.*/g);

      let entityKeyPattern = '/' + entity.urlTemplate.match(/\/.-*/g);
      ...
      for (let size in resolutions) {
        const bucket = coreConstants.S3_USER_ASSETS_BUCKET,
          entityKey = shortToLongUrl.replaceSizeInUrlTemplate(urlTemplate, size);
