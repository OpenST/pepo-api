...
class UserVideos extends VideoDeleteBase {
  ...
  async _performSpecificActivities(videoIds) {
    ...
    const promisesArray = [
      oThis._deleteVideoFromSocialGraph(videoIds),
  ...
  /**
   * Delete videos from Social Graph .
   *
   * @returns {Promise<void>}
   * @private
   */
  async _deleteVideoFromSocialGraph(videoIds) {
    const oThis = this,
      promises = [];

    for (let i = 0; i < videoIds.length; i++) {
      const promise = new VideoArangoModel().deleteEntryWithEdges({ videoId: videoIds[i] });
