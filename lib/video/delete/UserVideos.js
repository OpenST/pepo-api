...
class UserVideos extends VideoDeleteBase {
  ...
  async _deleteVideosFromChannels(videoIds) {
    ...
    for (let index = 0; index < videoIds.length; index++) {
      promises.push(oThis._deleteVideoFromChannels(videoIds[index]));
    }
  ...
  async _deleteVideoFromChannels(videoId) {
    ...
    while (true) {
      ...
      for (let index = 0; index < channelIds.length; index++) {
        const promise = new ChannelVideoIdsByChannelIdPaginationCache({ channelId: channelIds[index] }).clear();
