...
class ReplyVideos extends VideoDeleteBase {
  ...
  async _fetchAndSetIds() {
    ...
    if (oThis.replyDetailsIds && oThis.replyDetailsIds.length > 0) {
      ...
      //todo-replies: no index of creator user id
    ...
    for (let index = 0; index < dbRows.length; index++) {
      ...
      //todo-replies: what if pending state reply?
  ...
  async _markReplyDetailsDeleted(replyDetailsIds) {
    ...
    for (let index = 0; index < replyDetailsIds.length; index++) {
      ...
      parentIdToReplyDetailsIdsMap[parentId] = parentIdToReplyDetailsIdsMap[parentId] || [];
      parentIdToReplyDetailsIdsMap[parentId].push(replyDetailId);
    }

    for (const parentId in parentIdToReplyDetailsIdsMap) {
      const replyDetailIds = parentIdToReplyDetailsIdsMap[parentId];
    ...
    await VideoDetailModel.flushCache({ userId: oThis.userId, videoIds: parentVideoIds });
