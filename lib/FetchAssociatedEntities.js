...
class FetchAssociatedEntities {
  ...
  async perform() {
    ...
    promisesArray.push(oThis._fetchAtMentionUsers());
  ...
  async _fetchTagAndLinksFromText() {
    ...
    for (const textId in oThis.includesData) {
      ...
      for (let ind = 0; ind < textIncludes.length; ind++) {
        ...
        if (entity[0] == textIncludeConstants.tagEntityKindShort) {
          ...
          oThis.atMentionUserIds.push(+entity[1]);
        }
      }
    }
    ...
    oThis.atMentionUserIds = [...new Set(oThis.atMentionUserIds)];
  }
  ...
  _formatResponse() {
    ...
    let response = {
      ...
      atMentionUsersMap: oThis.atMentionUsers,
   ...
   * @private
   */
  async _fetchAtMentionUsers() {
    const oThis = this;

    if (oThis.atMentionUserIds.length <= 0) {
      return responseHelper.successWithData({});
    }

    const cacheRsp = await new UserMultiCache({ ids: oThis.atMentionUserIds }).fetch();
    ...
    for (let userId in cacheRsp.data) {
      ...
      oThis.atMentionUsers[userId] = user;
      if (profileImageId) {
  ...
  async _fetchTokenUsers() {
    ...
    const tokenUserRes = await new TokenUserDetailByUserIdsCache({ userIds: oThis.atMentionUserIds }).fetch();
