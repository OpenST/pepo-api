...
class FilterOutAtMentions {
  ...
  async _filterAtMentions() {
    ...
    let filteredNamesWithPrefix = oThis.textToFilter.match(/@\w+/g);
    ...
    for (let ind = 0; ind < filteredNames.length; ind++) {
      ...
      if (!CommonValidator.validateNonZeroInteger(userNamesToUserIdMap[filteredName])) {
        continue;
      }

      const userId = userNamesToUserIdMap[filteredName];
