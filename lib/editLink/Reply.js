...
class EditVideoLink extends EditLinkBase {
  ...
  async _fetchCreatorUserId() {
    ...
    console.log('From edit link  oThis.replyDetailId', oThis.replyDetailId);
    const replyDetailCacheResp = await new ReplyDetailsByIdsCache({ ids: [oThis.replyDetailId] }).fetch();

    console.log('From edit link ', replyDetailCacheResp);
  ...
  async _flushCache() {
    ...
    await ReplyDetailModel.flushCache({ replyDetailId: oThis.replyDetail.id, videoId: oThis.replyDetail.entityId });
  }
}
