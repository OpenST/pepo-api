...
class ReplyThreadNotification extends UserNotificationPublisherBase {
  ...
  async _publishForUserIds() {
    ...
    // Don't send notification if the last reply is of the same user
    if (
      oThis.replyThreadFollowerUserIds[0] == oThis.userId ||
      oThis.replyThreadFollowerUserIds[0] == oThis.parentVideoOwnerUserId
    ) {
      return;
    }

    // If user has been mentioned in replied video, don't publish reply thread notification.
    if (oThis.mentionedUserIds.includes(oThis.replyThreadFollowerUserIds[0])) {
      return;
    }
  ...
  _getHeadingVersionForActivityCentre(publishUserId) {
    ...
    if (oThis.parentVideoOwnerUserId == publishUserId) {
      return 2;
    }
