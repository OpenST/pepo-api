...
class ReplyThreadNotification extends UserNotificationPublisherBase {
  ...
  async _fetchReplyThreadFollowers() {
    ...
    for (let ind = 0; ind < allRepliesForParentVideoId.length; ind++) {
      const replyDetail = allRepliesForParentVideoId[ind],
        userId = replyDetail[replyDetailConstants.longToShortNamesMapForCache['creatorUserId']];

      if (!oThis.mentionedUserIds.includes(userId)) {
        userIdsArray.push(userId);
      }
    }

    oThis.replyThreadFollowerUserIds = basicHelper.uniquate(userIdsArray, -1);
  }
