...
class VideoDelegator {
  ...
  async perform() {
    ...
    await oThis._setSupporterReceipients();
  ...
  async _validateAndSanitize() {
    ...
    // TODO - channel - validate as array for mentioned user ids
    if (
  ...
  async _fetchUserSupporters() {
    ...
    oThis.supporterUserIds = await oThis._filterMutedUserIds(oThis.supporterUserIds);
  }
  ...
  _setMentionedUserNotificationRecipients() {
    ...
    // 2 cases - channel ids present or not. if present, we need to send channel name in the text.
    if (oThis.channelIds.length > 0) {
   ...
   * @private
   */
  async _setSupporterReceipients() {
    const oThis = this;

    if (!oThis.supporterUserIds.length) return;
    ...
    if (recipientCandidatesInSupporters.length && oThis.channelIds.length) {
