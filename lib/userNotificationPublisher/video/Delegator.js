...
class VideoDelegator {
  ...
  async _setRecipientUserIds() {
    ...
    if (oThis.mentionedUserIds.length > 0) {
      oThis.mentionedUserIds = basicHelper.arrayDiff(oThis.mentionedUserIds, [oThis.creatorUserId]);
    ...
    let channelUsers = [oThis.creatorUserId].concat(oThis.mentionedUserIds).concat(channelSupporterUserIds);
    ...
    if (oThis.channelIds.length > 0) {
      for (let channelId in oThis.channelIdToUserIdsMap) {
        ...
        if (channelUserIds.length > 0) {
          ...
          channelUsers = channelUsers.concat(channelUserIds);
        }
      }
    }
  }
