...
class VideoDelegator {
  ...
  async perform() {
    ...
    await oThis._checkUserMutedByAdmin();
   ...
   *
   * @sets oThis.isUserMuted
   *
   * @returns {Promise<void>}
   * @private
   */
  async _checkUserMutedByAdmin() {
    const oThis = this;

    const cacheResponse = await new UserMuteByUser2IdsForGlobalCache({ user2Ids: [oThis.creatorUserId] }).fetch();
    if (cacheResponse.isFailure()) {
      return Promise.reject(cacheResponse);
    }

    oThis.isUserMuted = cacheResponse.data[oThis.creatorUserId].all == 0 ? false : true;
  }
  ...
  async _setChannelRecipients() {
    ...
    if (oThis.isUserMuted || oThis.channelIds.length == 0) {
      return;
    }
