    ...
    while (true) {
      ...
      for (let index = 0; index < channelUsersCount; index++) {
        const channelUserId = channelUsersPaginationCacheRsp.data.userIds[index],
          channelUserRowId = channelUsersPaginationCacheRsp.data.channelUserDetails[channelUserId].id;

        if ((pageNo !== 1 && lastId <= channelUserRowId) || channelUserId == oThis.meetingHostUserId) {
          continue;
        }
