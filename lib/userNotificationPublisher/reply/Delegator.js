...
class ReplyDelegator extends UserNotificationPublisherBase {
  ...
  async _setRecipientUserIds() {
    ...
    let isVideoOwnerMentionedInReply = oThis.mentionedUserIds.includes(oThis.parentVideoOwnerUserId);
    let isVideoOwnerIsActor = oThis.parentVideoOwnerUserId == oThis.actor;
  ...
  async _publishForUserIds() {
    ...
    for (const notificationKind in oThis.notificationKindToPublishUserIds) {
      let publishUserIds = oThis.notificationKindToPublishUserIds[notificationKind];

      if(!publishUserIds.length) continue;
