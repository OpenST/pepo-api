...
class VideoAdd extends UserNotificationPublisherBase {
  ...
  async _publishForUserIds() {
    ...
    while (true) {
      ...
      const contributorUserCount = rsp.data.contributionUserIds.length;
      ...
      for (let index = 0; index < contributorUserCount; index++) {
        const contributorUserId = rsp.data.contributionUserIds[index];
      ...
      lastId = rsp.data.contributionUserIds[contributorUserCount - 1];
