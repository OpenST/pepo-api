...
class VideoAdd extends UserNotificationPublisherBase {
  ...
  async _filterMutedUserIds() {
    ...
    const userMuteByUser1IdsRespData = userMuteByUser1IdsResp.data,
      unmutedUserIds = []; // Create deep copy of publish user ids.

    // TODO feed done- optimize below. There are presently 2 loops inside this loop. n square.

    for (let index = 0; index < oThis.publishUserIds.length; index++) {
      const userId = oThis.publishUserIds[index];
    ...
    oThis.publishUserIds = unmutedUserIds;
  }
