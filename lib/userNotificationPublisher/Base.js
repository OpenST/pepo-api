...
class UserNotificationPublisherBase {
  ...
  async _insertIntoAggregatedNotificationsTable() {
    ...
    if (existingRow.length === 0) {
      ...
      logger.log('====== Existing amount ===== New amount to add::: ', existingAmount, newAmountToAdd);
      logger.log('====== Existing sender ===== New sender to add::: ', existingSenders, newSenderToAdd);
      ...
      logger.log(
        'All senders ===== Unique user ids ====== Final tx amount ::: ',
        allSenders,
        uniqUserIds,
        finalTxAmount
      );
      ...
      newExtraDataToBeInserted = Object.assign(oThis.payload.payload, newExtraDataToBeInserted);

      logger.log('===== Extra data to be inserted :::', JSON.stringify(newExtraDataToBeInserted));
