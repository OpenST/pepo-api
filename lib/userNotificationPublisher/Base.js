...
class UserNotificationPublisherBase {
  ...
  async _insertIntoNotificationHook() {
    ...
    let psRsp = await Promise.all(promiseArray);
  }
  ...
  async getSendTime(locationId, lastSendTimeInSec) {
    ...
    let currentEpochTimeInmilisecond = new Date().getTime(),
      ...
      nextDayTimeInSecond = currentEpochDayTimeInSecond + daySeconds;

    let sendTime =
      nextDayTimeInSecond - userTimezoneOffsetInSecond + aggregatedNotificationsConstants.defaultTimeToSendInSeconds;
  ...
  async _insertIntoAggregatedNotificationsTable() {
    ...
    //todo: check if user id is a creator and active
    ...
    if (existingRow.length === 0) {
      let sendTime = oThis.getSendTime(locationId, 0);
      ...
      if (existingRow[0].status != aggregatedNotificationsConstants.pendingStatus) {
        let sendTime = oThis.getSendTime(locationId, existingRow[0].sendTime);
        ...
        logger.log('Existing row is::: ', JSON.stringify(existingRow[0]));
        ...
        existingExtraData.payload.amountInEth = aggregationPayload.payload.amountInEth;

        logger.log('Current Payload is ::: ', JSON.stringify(aggregationPayload));
        logger.log('New ExtraData is::: ', JSON.stringify(existingExtraData));
