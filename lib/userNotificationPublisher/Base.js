...
class UserNotificationPublisherBase {
  ...
  async enqueueUserNotification() {
    const oThis = this;
    console.log('oThis.publishUserIds ===1234567===', oThis.publishUserIds);

    await oThis._insertIntoNotificationHook();

    const promises = [];

    console.log('oThis.publishUserIds ===22222222===', oThis.publishUserIds);
  ...
  async _insertIntoNotificationHook() {
    ...
    while (publishUserIds.length > 0) {
      const currentUserIds = publishUserIds.splice(0, notificationHookConstants.senderBatchSize);
