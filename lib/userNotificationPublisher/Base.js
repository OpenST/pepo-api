...
class UserNotificationPublisherBase {
  ...
  constructor() {
    ...
    oThis.payload = {};
    oThis.userDeviceIds = [];
    oThis.deviceIdToUserDeviceDetails = {};
  }
  ...
  async _insertIntoNotificationHook() {
    ...
    let incrementUnreadCountForUserIds = [];
    ...
    while (oThis.userDeviceIds.length > 0) {
      ...
      for (let ind = 0; ind < userDeviceIds.length; ind++) {
        let userId = oThis.deviceIdToUserDeviceDetails[userDeviceIds[ind]];
        incrementUnreadCountForUserIds.push(userId);
      }
    ...
    incrementUnreadCountForUserIds = [...new Set(incrementUnreadCountForUserIds)];
    logger.log('incrementUnreadCountForUserIds', incrementUnreadCountForUserIds);

    while (incrementUnreadCountForUserIds.length > 0) {
      const batchUserIds = incrementUnreadCountForUserIds.splice(0, notificationHookConstants.hookSenderBatchSize);

      // Increment unread notifications count for these users.
  ...
  async _getUserDeviceIds() {
    ...
    for (let id in userDevicesResponseData) {
      ...
      if (userDeviceRow.status === userDeviceConstants.activeStatus) {
        ...
        oThis.deviceIdToUserDeviceDetails[id] = userDeviceRow;
      }
    }
  }
