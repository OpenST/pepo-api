...
class UserNotificationPublisherBase {
  ...
  async _insertIntoAggregatedNotificationsTable() {
    ...
    let userId = oThis.publishUserIds[0];
    ...
    const profileElementsData = cacheRsp.data,
      locationId = profileElementsData[userId][userProfileElementConst.locationIdKind].data;
    ...
    if (existingRow.length === 0) {
      oThis.payload.senders = oThis.payload.actorIds;
