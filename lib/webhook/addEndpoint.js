...
class AddEndpointWebhook {
  ...
  async _validateAndSanitizeParams() {
    ...
    for (let index = 0; index < oThis.eventTopics.length; index++) {
      ...
      eventTopicObj = eventTopicObj.topic_kind.toLowerCase().trim();

      if (!webhookSubscriptionConstants.invertedTopicKinds[eventTopicObj.topic_kind]) {
  ...
  async createEndpoint() {
    ...
    if (oThis.endpoint) {
      ...
      let secret_salt = oThis.secretSalt.CiphertextBlob;
