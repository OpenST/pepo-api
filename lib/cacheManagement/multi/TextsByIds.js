/**
 * Multi Cache for texts by ids
 *
 *
 * @module lib/cacheManagement/multi/TextsByIds
 */
...
class TextsByIds extends CacheMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.ids.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + '_cmm_t_id_' + oThis.ids[i]] = oThis.ids[i];
    }
  }
  ...
  async fetchDataFromSource(cacheMissIds) {
    ...
    let fetchByIdsRsp = await new TextModel().fetchByIds(cacheMissIds);
