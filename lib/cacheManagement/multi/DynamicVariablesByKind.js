/**
 * Multi Cache for dynamic variables by kinds.
 *
 *
 * @module lib/cacheManagement/multi/DynamicVariablesByKind
 */
...
class DynamicVariablesByKind extends CacheMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.kinds.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + '_cmm_dgc_k_' + oThis.kinds[i]] = oThis.kinds[i];
    }
  }
  ...
  async fetchDataFromSource(cacheMissKinds) {
    ...
    let fetchByKindsRsp = await new DynamicVariablesModel().getForKinds(cacheMissKinds);
