/**
 * Multi Cache for user action details by user ids.
 *
 * @module lib/cacheManagement/multi/UserActionDetailsByUserIds
 */
...
class UserActionDetailsByUserIds extends CacheMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.entityIdentifiers.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + `_cmm_uad_${oThis.userId}_by_uid_` + oThis.entityIdentifiers[i]] =
        oThis.entityIdentifiers[i];
    }
  }
