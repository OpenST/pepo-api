...
class TokenUserByOstUserIds extends CacheMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.ostUserIds.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + '_cmm_tud_uids_' + oThis.ostUserIds[i]] = oThis.ostUserIds[i];
    }
  }
  ...
  async fetchDataFromSource(cacheMissIds) {
    ...
    if (tokenUserObj.isSuccess()) {
      return responseHelper.successWithData(tokenUserObj.data);
    }
