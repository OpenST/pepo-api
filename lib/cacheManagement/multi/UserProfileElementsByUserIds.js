...
class UserProfileElementsByUserIds extends CacheMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.usersIds.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + '_cmm_upe_id_' + oThis.usersIds[i]] = oThis.usersIds[i];
    }
  }
  ...
  async fetchDataFromSource(cacheMissIds) {
    const fetchByIdsRsp = await new UserProfileElementModel().fetchByUserIds(cacheMissIds);
