...
class UserStatByUserIds extends CacheMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.userIds.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + '_cmm_us_uid_' + oThis.userIds[i]] = oThis.userIds[i];
    }
  }
  ...
  async fetchDataFromSource(cacheMissIds) {
    ...
    let fetchByUserIdsRsp = await new UserStatModel().fetchByUserIds(cacheMissIds);
