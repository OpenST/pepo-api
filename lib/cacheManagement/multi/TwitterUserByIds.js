...
class TwitterUserByIds extends CacheMultiBase {
  /**
   * constructor
   *
   * @param params
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.ids.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + '_cmm_twu_id_' + oThis.ids[i]] = oThis.ids[i];
    }
  }
  ...
  async fetchDataFromSource(cacheMissIds) {
    ...
    let fetchByIdsRsp = await new TwitterUserModel().fetchByIds(cacheMissIds);

    return responseHelper.successWithData(fetchByIdsRsp);
  }
}

module.exports = TwitterUserByIds;
