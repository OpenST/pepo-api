...
class UserProfile extends CacheMultiBase {
  ...
  async fetchDataFromSource(cacheMissIds) {
    ...
    for (let userId in userIdProfileElementsMap) {
      ...
      for (let ind = 0; ind < profileElements.length; ind++) {
        ...
        let profileElementDetailRsp = await oThis._getElementKindDetails(element);

        Object.assign(profileInfo, profileElementDetailRsp.data);
      }
  ...
  async _getElementKindDetails(profileElement) {
    ...
    return responseHelper.successWithData(elementDetails);
  }
