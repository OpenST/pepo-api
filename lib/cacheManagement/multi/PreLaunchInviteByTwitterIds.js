...
class PreLaunchInviteByTwitterIds extends CacheMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.twitterIds.length; i++) {
      oThis.cacheKeys[oThis._cacheKeyPrefix() + '_cmm_pli_tid_' + oThis.twitterIds[i]] = oThis.twitterIds[i];
    }
  }
  ...
  async fetchDataFromSource(cacheMissIds) {
    ...
    let fetchByTwitterIdsRsp = await new PreLaunchInviteModel().fetchByTwitterIds(cacheMissIds);
