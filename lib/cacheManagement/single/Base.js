...
class CacheManagementBase {
   ...
   *       oThis._setCacheImplementer() to set oThis.cacheImplementer.
   *
   * @returns {Promise<*>}
   */
  async clear() {
  ...
  /**
   * Subscribe for connection errors
   *
   * @private
   */
  _subscribeForErrors() {
    ...
    // TODO - @Santhosh - following lines give function called on null error.

    // oThis.cacheImplementer.client.on('issue', async function(details) {
    //   const errorMessage = 'Server ' + details.server + 'seems to be down: ' + details.messages.join('');
    //   logger.error(errorMessage);
    //
    //   const errorObject = responseHelper.error({
    //     internal_error_identifier: 'l_cm_s_b_3',
