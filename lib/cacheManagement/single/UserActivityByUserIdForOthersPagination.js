/**
 * Module for caching user activity rows for other user.
 *
 * @module lib/cacheManagement/shared/UserActivityByUserIdForOthersPagination
 */
...
/**
 * Class to get user activity rows from cache for other user.
 *
 * @class UserActivityByUserIdForOthersPagination
 */
class UserActivityByUserIdForOthersPagination extends CacheSingleBase {
  /**
   * Constructor
   *
   * @param {object} params
  ...
  async _setPageCacheKey() {
    ...
    await oThis._subscribeForErrors();
    ...
    if (!versionDetail || versionCacheResponse.isFailure() || versionDetail.limit !== oThis.limit) {
      ...
      // NOTE: Set this key only when paginationTimestamp is null. If it's set on every page request. There is a possibility of some data
      // not being included in any page.
      if (!oThis.paginationTimestamp) {
        await oThis.cacheImplementer.setObject(oThis._versionCacheKey, versionDetail, oThis.cacheExpiry);
      }
    }

    oThis.cacheKey = oThis.baseCacheKey + '_' + versionDetail.v + '_' + oThis.paginationTimestamp;
  }
  ...
  async fetchDataFromSource() {
    ...
    let fetchUsersRsp = await new UserActivityModel()._otherUserActivityIds({
      limit: oThis.limit,
...
module.exports = UserActivityByUserIdForOthersPagination;
