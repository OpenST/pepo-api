...
class UpdateStats {
  ...
  async _markFeedPopularIfEligible() {
    ...
    if (
      dynamicThreshold &&
      oThis.videoDetails.totalAmount &&
      new BigNumber(oThis.videoDetails.totalAmount + oThis.totalAmount).gte(new BigNumber(dynamicThreshold))
    ) {
      ...
      if (feedResp[0] && feedResp[0].id) {
        let formattedFeedRow = new FeedModel().formatDbData(feedResp[0]);
  ...
  async _updateVideoDetails() {
    ...
    let updateResponse = await new VideoDetailsModel().updateByVideoId({
      videoId: oThis.videoId,
      totalAmount: oThis.totalAmount,
  ...
  async _updateReplyDetails() {
    ...
    let updateResponse = await new ReplyDetailsModel().updateByReplyDetailId({
    ...
    await ReplyDetailsModel.flushCache({ replyDetailId: oThis.replyDetailId });
  }
