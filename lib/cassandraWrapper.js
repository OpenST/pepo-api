...
// a.execute('DROP TABLE user_notifications ',[], {}).then(console.log);
...
//
//
// s = "CREATE TABLE user_notification_center_visit_details \n\
// ( \n\
//   user_id bigint,\n\
...
class CassandraSdkWrapper {
  ...
  constructor() {
    ...
    oThis.client = new cassandra.Client({
      contactPoints: ['127.0.0.1:9042'],
      localDataCenter: 'datacenter1',
      keyspace: 'pepo_api_development'
    });
  ...
  _subscribeForLogging() {
    ...
    oThis.client.on('log', function(level, className, message, furtherInfo) {
      let errMsg = `log event: ${level} -- ${message}`;
