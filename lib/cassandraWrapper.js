...
class CassandraSdkWrapper {
   ...
   * @returns {Promise<*>}
   */
  async eachRow(query, params, options = { prepare: true }, rowCallback) {
    const oThis = this;
      ...
      oThis.client.eachRow(
        ...
        function(n, row) {
          ...
          if (rowCallback) {
            ...
            //endcallback will always return the raw rows received from Cassandra.
            rowCallback(n, row);
          }
        },
        function(err, result) {
          logger.debug('End Cassandra Calllback');
          logger.info('(' + (Date.now() - preQuery) + ' ms)', query, params);
