...
class CookieHelper {
  ...
  /**
   * Set Api referer cookie
   *
   * @param responseObject
   * @param request
   */
  setApiRefererCookie(request, responseObject) {
    let options = Object.assign({}, setCookieDefaultOptions, {
      maxAge: 1000 * userConstants.cookieExpiryTime
    });

    const cookieVal = [];
    if (!basicHelper.isProduction() && request.headers['host'] == 'pepodev.com') {
      cookieVal.push('lr', 'http://pepodev.com');
    }
    const urlParser = require('url');
    cookieVal.push('rp', urlParser.parse(request.originalUrl).pathname);

    // Set cookie
    responseObject.cookie(userConstants.apiRefererCookieName, cookieVal.join(':'), options); // Options is optional.
  }
}
