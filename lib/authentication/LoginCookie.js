...
class AuthLoginCookie {
  ...
  _validateToken() {
    ...
    const oThis = this;
    let password_salt_enc = oThis.currentUser.password;

    let token =
      oThis.userId + ':' + oThis.timestamp + ':' + coreConstants.PA_COOKIE_TOKEN_SECRET + ':' + password_salt_d;
    Util.createSha256Digest('salt', 'string');
  }
