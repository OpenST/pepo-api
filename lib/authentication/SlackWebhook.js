...
class AuthSlackWebhook {
  ...
  async _fetchAndValidateAdmin() {
    ...
    if (
      ...
      !CommonValidator.validateNonEmptyObject(adminBySlackIdCacheRsp.data) ||
    ...
    if (cacheResponse.isFailure() || !CommonValidator.validateNonEmptyObject(cacheResponse.data)) {
  ...
  async _validateSignature(requestTimestamp, version, signature) {
    ...
    const generatedSignature = crypto
      .createHmac('sha256', coreConstants.PEPO_SLACK_SIGNING_SECRET)
