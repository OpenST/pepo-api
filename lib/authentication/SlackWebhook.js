...
class AuthSlackWebhook {
  ...
  async _validateRawBodyParams() {
    ...
    if (!CommonValidator.validateNonEmptyObject(oThis.rawBody)) {
      return oThis._unauthorizedResponse('l_a_sw_vrbp_1');
    }

    if (!CommonValidator.validateString(oThis.rawBody.payload)) {
      return oThis._unauthorizedResponse('l_a_sw_vrbp_2');
    }

    let domain = oThis.webhookParams.team.domain,
      channelName = oThis.webhookParams.channel.name,
      apiAppId = oThis.webhookParams.api_app_id,
      isValidApiAppId = slackConstants.allowedApiAppIds.indexOf(apiAppId) !== -1,
      isValidChannelName =
    ...
    if (!isValidApiAppId || !isValidChannelName || !isValidSlackDomain) {
      return oThis._unauthorizedResponse('l_a_sw_vrbp_3');
    }
  }
  ...
  async _fetchAndValidateAdmin() {
    const oThis = this,
      adminSlackId = oThis.webhookParams.user.id;

    logger.log('Fetching admin.');

    let AdminBySlackIdCacheRsp = await new AdminBySlackIdCache({ slackId: adminSlackId }).fetch();
    if (AdminBySlackIdCacheRsp.isFailure() || !CommonValidators.validateNonEmptyObject(AdminBySlackIdCacheRsp.data)) {
      return oThis._unauthorizedResponse('l_a_sw_fava_1');
    }

    let adminId = cacheResponse.data.id;
  ...
  async _valiadateRequestHeaders() {
    ...
    if (
      ...
      requestTimestamp < currentTimestamp - 5 * 60
    ) {
