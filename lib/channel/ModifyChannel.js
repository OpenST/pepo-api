...
class ModifyChannel {
  ...
  async _associateAdminsToChannel() {
    ...
    const channelAdminIdsMap = await new ChannelUserModel().fetchAdminProfilesByChannelId([oThis.channelId]),
      channelAdminIds = channelAdminIdsMap[oThis.channelId];
  ...
  async _associateTagsToChannel() {
    ...
    for (let index = 0; index < oThis.channelTagNames.length; index++) {
      const sanitizedChannelTagName = oThis.channelTagNames[index].trim();
