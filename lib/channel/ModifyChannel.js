...
class ModifyChannel {
   ...
   * Validate parameters.
   *
   * @sets oThis.channelNewAdminUserData
   *
   * @returns {Promise<void>}
   * @private
   */
  async _validateParams() {
    ...
    for (let caui = 0; caui < oThis.channelAdminUserIds.length; caui++) {
      ...
      if (!CommonValidators.validateNonEmptyObject(oThis.channelNewAdminUserData[adminUserId])) {
  ...
  async _associateTagsToChannel() {
    ...
    for (let existingTagId in oThis.channelExistingTagIdsMap) {
      if (!oThis.newTagIdsToBeAssociatedMap[existingTagId]) {
        tagsToBeRemoved.push(existingTagId);
      }
    }
    for (let newTagId in oThis.newTagIdsToBeAssociatedMap) {
