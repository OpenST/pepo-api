...
/**
 * Class to block user from a channel.
 *
 * @class BlockUserFromChannel
 */
class BlockUserFromChannel extends ServiceBase {
  /**
   * Constructor to block user from a channel.
   *
   * @param {object} params
  ...
  async _fetchChannelUser() {
    ...
    if (!CommonValidators.validateNonEmptyObject(oThis.channelUserObj)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'a_s_c_u_b_fcu_1',
          api_error_identifier: 'entity_not_found',
          debug_options: {
            channelId: oThis.channelId,
            userId: oThis.userIdToBlock
    ...
    if (oThis.channelUserObj.status === channelUsersConstants.inactiveStatus) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'a_s_c_u_b_fcu_2',
    ...
    if (oThis.channelUserObj.status === channelUsersConstants.blockedStatus) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'a_s_c_u_b_fcu_3',
...
module.exports = BlockUserFromChannel;
