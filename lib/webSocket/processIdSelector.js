...
class ProcessIdsSelector {
  ...
  async perform() {
    ...
    let rmqIdWithLeastLoad = oThis._selectRabbitMqWithLeastLoad();
    let cronParams = {
  ...
  async _fetchSocketRmqNodes() {
    ...
    console.log('socketRmqConfig-----', socketRmqConfig);

    let socketRmqs = socketRmqConfig.clusters;
