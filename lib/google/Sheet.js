...
class Sheet {
  ...
  /**
   * Generate new tab in sheet
   *
   * @param spreadsheetId
   * @param title
   * @returns {Promise<void|*|result>}
   */
  async generateNewTabInSheet(spreadsheetId, title) {
    const oThis = this,
      sheets = googleapis.google.sheets('v4');
    ...
    const sheetResp = await sheets.spreadsheets.batchUpdate(request).catch(function(err) {
      return oThis._triggerPagerduty(err);
    });

    if (sheetResp && sheetResp.data && sheetResp.data.replies) {
      return responseHelper.successWithData({ gid: sheetResp.data.replies[0].addSheet.properties.sheetId });
    } else {
      return sheetResp;
    }
  }
